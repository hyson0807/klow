...
- **국가별 2kg 고정 배송 요율:** 제품 가격은 **배송 국가별 2kg 고정 요율**을 쓴다 — **제품 무게는 가격에 영향 없음**(`Product.weightG` 는 legacy 호환용, EFS 송장도 입고 시 재측정). 요율 정본은 `ShippingCountry.{efs,ems,dhl}RateKrw`(= `ShippingRate` 2kg 티어 올림으로 동기화한 대표값 + 가용 플래그, 파생 미러 — `syncCountryColumn`/`rate-writer` 로만 갱신). **캐리어는 2kg 최저가** — EFS·EMS 둘 다 가능하면 더 싼 쪽(동점 EFS, EFS 제외구역이면 EMS), 둘 다 불가면 DHL(`pickCheapestCarrier`). 주문 시 `shipping.service.resolveFixedRate(iso2, addr)` 가 `(캐리어, 2kg 요율)` 1회 산출 → **판매가 마크업 = `요율/2`(전 아이템 공통), 결제 배송비 = `요율/2 × 브랜드수`**(한 브랜드 = 한 송장). 캐리어는 국가/주소 단위라 전 브랜드 동일하지만 송장 발급 호환 위해 `Order.shippingCarrierByBrand`(JSON) 에 스냅샷. 무게 티어 테이블 `ShippingRate(carrier,iso2,weightG,rateKrw)`·엑셀 업로드·어드민 무게×캐리어 매트릭스는 **유지**(2kg 대표값 도출 + 크리에이터 시딩 박스 비용 산정 — 시딩은 박스 실측 무게 기반 `loadCheapestRatePlanner`/`resolveRatesForWeight`, 제품 가격과 별개). 시드 `npm run seed:shipping-rates`(엑셀→`prisma/data/*_rates.json`), 미러 1kg→2kg 백필은 데이터 마이그레이션 `20260619073348_resync_2kg_mirror`(운영은 `prisma migrate deploy` 가 자동 적용). 자세히는 [`docs/pricing-model.md`](./docs/pricing-model.md) + `klow_server/docs/modules/shipping.md`.
