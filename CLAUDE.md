...
├── KLOW/          ← Legacy mobile webapp (Next.js 14, still uses data/mock.ts; superseded by klaw_web)
├── klaw_web/      ← Public mobile webapp   (Next.js 14, port 3001, reads /v1/* via TanStack Query)
...
- **NEVER** stage or commit files inside `KLOW/`, `klaw_web/`, `klow_admin/`, or `klow_server/` from the workspace root.
...
`klow_server` (NestJS, port 4000) is the single source of truth for the database (Neon Postgres via Prisma) and file storage (Cloudflare R2). It exposes two URL surfaces: `/admin/*` for full CRUD by `klow_admin`, and `/v1/*` for read-only access by `klaw_web` (and later, mobile/native apps). Both `klaw_web` and `klow_admin` are pure UI clients — neither has Prisma installed. The legacy `KLOW/` project is still wired to `data/mock.ts` and is being phased out in favor of `klaw_web`. See `docs/architecture.md` for the full picture, including module conventions, data model, and request flow examples.
...
| klaw_web pages | `klaw_web/src/app/` (feed, product, creator, shop, discover, my) |
| klaw_web API client | `klaw_web/src/lib/api.ts` |
| klaw_web TanStack Query hooks | `klaw_web/src/hooks/` |
...
- **klaw_web port:** `3001` (Next.js dev, chosen to avoid the admin)
...
- **CORS:** `klow_server/src/main.ts` already whitelists `http://localhost:*` via regex, so both admin (3000) and klaw_web (3001) work out of the box. Swap to an explicit origin list before deploy.
...
cd klaw_web && npm run dev             # http://localhost:3001
```
