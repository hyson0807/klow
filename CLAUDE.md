...
- **국가별 2kg 고정 물류비:** 제품 가격은 **배송 국가별 2kg 통합 물류비**를 쓴다 — **제품 무게는 가격에 영향 없음**(`Product.weightG` 는 legacy 호환용, EFS 송장도 입고 시 재측정). 가격 정본은 `ShippingCountry.productLogisticsCostKrw`(운영팀이 **캐리어 비교·유류할증료·특별운송수수료를 미리 반영한** 2kg 통합값, 엑셀 `KLOW_가격표`), 캐리어 정본은 `ShippingCountry.productCarrier`(직통 10국=EFS / 나머지 88국=EMS) — **런타임 비교·할증 가산 없음**. 주문 시 `shipping.service.resolveFixedRate(iso2, addr)` 가 `(캐리어, 물류비)` 1회 반환 → **판매가 마크업 = `물류비/2`(전 아이템 공통), 결제 배송비 = `물류비/2 × 브랜드수`**(한 브랜드 = 한 송장). `productLogisticsCostKrw`/`productCarrier` 미설정(표 외 DHL-전용국 등)이면 **구매 차단**, EFS 캐리어 국가라도 주소가 EFS 제외구역이면 **폴백 없이 차단**. 캐리어는 국가 단위라 전 브랜드 동일하지만 송장 발급 호환 위해 `Order.shippingCarrierByBrand`(JSON) 에 스냅샷. 어드민 **물류비용** 탭(`klow_admin /product-logistics-cost`)에서 국가별 수기 편집, 초기 적재 시드 `npm run seed:product-logistics-cost`(`prisma/data/product_logistics_cost.json`). klow_web `resolveCarrierAndRate`·klow_brand `productRateKrw` 가 동일 값을 미러. **미러 컬럼 `ShippingCountry.{efs,ems,dhl}RateKrw`·무게 티어 `ShippingRate`·kg당 추가요금·엑셀 업로드·어드민 무게 매트릭스·`pickCheapestCarrier` 는 크리에이터 시딩 박스 비용 산정 전용으로 유지**(박스 실측 무게 기반 `loadCheapestRatePlanner`/`resolveRatesForWeight`, 제품 가격과 별개). 자세히는 [`docs/pricing-model.md`](./docs/pricing-model.md) + `klow_server/docs/modules/shipping.md`.
