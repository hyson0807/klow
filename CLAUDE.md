...
- **무게별 배송 요율 (ShippingRate):** 배송 요율은 국가×캐리어×무게 구간(grams)별 곡선(`ShippingRate(carrier,iso2,weightG,rateKrw)`). `Product.weightG`(신규 등록 필수, legacy 1kg) 가 조회 기준. 요율은 올림(round-up)으로 고르고 최대 티어 초과 시 결제 차단. **캐리어는 무게별 최저가** — EFS·EMS 둘 다 가능하면 그 무게에서 더 싼 쪽(동점 EFS, EFS 제외구역이면 EMS), 둘 다 불가면 DHL(`pickCheapestCarrier`). EFS 가 가능해도 EMS 가 더 싸면 EMS. 브랜드마다 캐리어가 갈릴 수 있어 주문 시 `Order.shippingCarrierByBrand`(JSON) 에 브랜드별 캐리어를 스냅샷하고 송장 발급이 그대로 쓴다. **판매가 마크업 = `요율(제품 1개 무게)/2`, 결제 배송비 = Σ_브랜드 `요율(브랜드 제품 무게합산)/2`** (한 브랜드 = 한 송장). `ShippingCountry.{efs,ems,dhl}RateKrw` 는 1kg 대표값+가용 플래그(파생 미러, `ShippingRate` 로만 갱신). 시드 `npm run seed:shipping-rates`(엑셀→`prisma/data/*_rates.json`), 어드민은 엑셀 재업로드 + 무게×캐리어 셀 편집. 자세히는 [`docs/pricing-model.md`](./docs/pricing-model.md) + `klow_server/docs/modules/shipping.md`.
