...
- **Auth (brand):** Email + Password (argon2id) + 이메일 OTP **+ Google OAuth**. `BrandUser` / `BrandSession` 테이블 + httpOnly 쿠키(`klow_brand_sid`, 7일 기본). `BrandGuard`가 세션 검증(klow_server `src/modules/brand-auth/`). 공개 자체 가입(invitation 없음, TOTP 없음 — 마찰 최소화). klow_brand 가 `/v1/brand/auth/*` 호출. EmailVerification 테이블의 `purpose` 컬럼에 `brand-signup-otp` / `brand-signup-token` 추가 사용. Google OAuth 는 `klow_web` 과 동일한 `GOOGLE_CLIENT_ID/SECRET` 을 공용으로 쓰고 callback URL 만 `GOOGLE_BRAND_CALLBACK_URL` 로 분리 (`/v1/brand/auth/google` → `/google/authorize` → `/google/callback`). `BrandUser.passwordHash` 는 nullable, `googleId` 는 unique. 콜백 후 `BRAND_FRONTEND_URL` 로 redirect.
- **Brand 입점 워크플로우:** `Brand` 모델에 `status`(draft/pending/approved/rejected), `homepageUrl`, `targetCountries[]`, `submittedById`/`submittedAt`/`approvedAt`/`approvedById`/`rejectionReason` 추가. `Product` 도 `status`(pending/approved/rejected) + `submittedById`/`submittedAt`/`rejectionReason` 추가. 기존 행은 모두 `status='approved'` 로 백필 마이그레이션. **노출과 구매가 분리된 정책** — 공개 surface(`/v1/brands`, `/v1/products`, `/v1/discover`, `/v1/shop/today`, `/v1/creators/:id/products`, `/v1/videos/:id` 의 products include)는 `PUBLIC_PRODUCT_WHERE`(rejected 만 차단, draft/pending 브랜드와 pending 제품 모두 통과)로 필터링되어, klow_brand 가입 후 제품을 등록하기만 하면 어드민 승인 없이도 즉시 브랜드관과 제품이 노출된다. 반면 카트 담기·결제는 `isPurchasable()` / `PURCHASABLE_PRODUCT_WHERE`(둘 다 approved-only)가 별도로 막아, 심사 제출 → 어드민 승인이 끝나야 비로소 구매가 열린다. `/admin/brand-applications/*` 는 어드민 전용 검수 큐(= 판매 활성화 큐). 승인은 트랜잭션 — Brand.status='approved' + 선택된 Product.status='approved' 동시 처리. 신청자 brandUser 는 `BrandUser.brandId` 로 자기 브랜드와 연결되어 승인 후에도 추가 제품을 `/v1/brand/products` 로 올릴 수 있고, 추가 제품은 모두 `status='pending'` 으로 시작하여 어드민 별도 검수(노출은 즉시, 구매는 승인 후).
