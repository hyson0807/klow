...
- **국내 자체배송 `DOMESTIC` (2026-08-13)**: `ShippingCarrier` 의 다섯째 값으로, **브랜드가 자기 국내 택배로 직접 보내는 목적지**를 뜻한다. 현재 유일한 국가가 **KR** 이고, 그 `ShippingCountry` 행은 `carriers_by_country.json`(EFS/EMS/DHL 요율 엑셀 산출물, KR 은 출발지라 애초에 없다)이 아니라 **`prisma/seed/seed-shipping-countries.ts` 의 `DOMESTIC_COUNTRIES` 블록이 소유**한다 — `enabled:false`(일반 스토어 차단) · `productCarrier:'DOMESTIC'` · `seedingCarrierSplitWeightG:null` 을 **update 절에서도 강제**하므로 재시드가 곧 복구 경로다.
  - ⚠️ **`ProductCarrierEnum` 에서 제외**한다(`.exclude(['DHL','DOMESTIC'])`). 그 목록은 `ShippingCarrierEnum` 파생이라 그냥 두면 자동으로 어드민 선택지가 되고, 아무 국가나 DOMESTIC 으로 바뀌는 순간 그 나라 주문이 **조용히 EFS 송장 없이** 발송 처리된다. 어드민 배송비용 탭도 이 행을 셀렉트가 아니라 읽기전용 `국내배송` 칩으로 그린다(옵션에 없는 값이라 빈 셀렉트로 뜨고, 아무 변경이나 실제 캐리어로 덮어쓰기 때문).
  - ⚠️ **`pickCarrierForWeight` 는 DOMESTIC 을 무게 분기보다 먼저 반환**한다 — 누가 KR 에 분기 무게를 넣어도 국내 건이 EFS 로 라우팅돼 실제 송장이 발급되면 안 된다.
  - ⚠️ **`resolveProductShipping`(일반 주문)은 DOMESTIC 이 나오면 throw** 한다. `loadEnabledCountry` 가 이미 막지만, 불변식을 그 함수 안에 둬야 배송지원을 잘못 켜도 새지 않는다.
  - **요율표(`SeedingRate`) 행이 없다** — 국내 배송은 KLOW 가 요율을 갖지 않고(브랜드가 자기 택배비를 낸다) 요율표는 수출 아웃바운드 전용이다. 그래서 `supportedCountries()` 가 교집합이 아니라 **합집합**이다(아래 seeding 모듈).
  - 송장 생명주기(EFS 미호출·취소·발송처리=완료)는 [shipments](./shipments.md) 의 국내 자체배송 항목 참고.
...
- **시딩 통관 신고가 `seedingDeclaredValueUsd` (2026-08-13)**: 시딩 EFS 송장의 통관 신고**총액**(USD 센트). 목적국 면세 한도(de minimis)보다 조금 낮게 국가별로 세팅해두면 claim 시점에 그 값이 EFS 26번 총액이 되고 단가(24-11)는 `floor(총액 ÷ 수량)` 로 파생된다 — **박스에 몇 개를 담아도 총액이 고정**돼 한도를 넘지 않는다(→ [seeding](./seeding.md)). `null` = 미설정이라 링크 발급 시 랜덤($8.50~$12.50)으로 폴백하므로 **채우기 전까지 동작 변화가 없다**(마이그레이션은 nullable ADD COLUMN, 백필 없음). 목록 인라인 편집은 **달러 정수 입력 → ×100 센트 저장**이고 **빈 값은 `null`**(미설정)이다 — 배송비용 탭의 청구 수수료가 빈 값을 ₩1,000 으로 채우는 것과 의도적으로 다르다. 일반 주문은 실제 판매가를 신고하므로 이 컬럼과 무관하다. ⚠️ **공개 응답에 실으면 안 된다** — 아래 조회 항목 참고.
  - **엑셀 왕복 (2026-08-13)**: 국가 설정 탭에서 `GET …/export` 로 `시딩신고가` 시트(전 국가 1행씩, 값은 **달러**)를 받아 고친 뒤 `POST …/import/preview` → 국가 선택 → `…/import/apply` 로 반영한다. apply 는 **같은 파일을 재파싱**해 "미리보기에서 본 것"과 "저장되는 것"이 갈라지지 않게 한다(요율표 임포트와 같은 규약). ⚠️ **엑셀에 없는 행은 미변경이지만, 행이 있는데 칸이 비면 `null`(미설정)로 지운다** — 무손실 왕복과 "랜덤 폴백으로 되돌리기"를 위해서고, 배송비용 탭 `청구수수료` 시트의 "빈 칸 = 미변경"과 **의도적으로 반대**다(그쪽은 기본값 ₩1,000 이 있어 '미설정' 상태가 없다). 미리보기가 `clear` 를 별도 색으로 구분하는 이유. 숫자로 못 읽은 칸·상한 초과·모르는 ISO2 는 저장하지 않고 보고한다(조용히 버리면 "적용했는데 안 바뀌었다"가 된다). 라이터·파서 한 쌍은 `shipping/seeding-declared-xlsx.ts` 가 소유하고(포맷 불변식이 이 기능의 전부라 한 파일에 둔다) 회귀 가드는 `__tests__/seeding-declared-xlsx.spec.ts`.
  - ⚠️ **`@Get('export')` 는 반드시 `@Get(':iso2')` 앞**에 선언한다 — 뒤면 `iso2='export'` 로 먹힌다(`admin-seeding-rate.controller.ts` 와 같은 함정).
  - **초기 적재 시드**: `prisma/data/seeding_declared_values.json`(각국 **소액면세 한도** + `vatFromZero` + `confidence`) → `npm run seed:seeding-declared-values`(**기본 dry-run**, `-- --apply` 로 반영, `--overwrite` 없으면 **미설정 국가만** 채워 어드민 수동 편집을 보존한다. 멱등). 파생 규칙은 스크립트가 소유한다 — **`min(한도 − $5, $100)` → $5 단위 내림**, `vatFromZero` 면 **$10 고정**. JSON 에는 "그 나라가 정한 한도"만 적고 "우리가 얼마로 신고할지"는 적지 않는다(한도가 바뀌면 숫자 하나만 고치면 되고 근거가 남는다).
  - ⚠️ **상한 $100 이 필요한 이유**: 면세 한도가 높은 나라(미국 $800 등)에 맞춰 신고하면 무료 샘플 박스의 실제 가치와 동떨어져 **오히려 검사를 부르고 허위신고**가 된다.
  - ⚠️ **`vatFromZero` 가 별도 축인 이유**: de minimis 는 대개 *관세* 면제 한도이고 **부가세는 별개**다 — EU 는 관세 €150 면제지만 VAT 는 €0 부터(2021 IOSS), 영국도 관세 £135 / VAT £0 부터, 호주는 관세 A$1,000 면제지만 GST 는 A$0 부터다. 이런 나라는 신고가를 올릴수록 **수령인(크리에이터)이 낼 세금만** 늘어나므로 낮게 고정한다. 한도가 회피 불가능하게 낮은 나라(인도네시아 US$3)도 같은 칸에 묶인다.
  - ⚠️ **JSON 은 규제값 스냅샷이라 낡는다.** `confidence: 'unverified'` 인 국가(**US** — $800 de minimis 가 2025년 중단, CN/RU/TR — 최근 수차례 변경)는 값을 비워둬 스킵되고 랜덤 폴백이 유지된다. 수록되지 않은 국가도 전부 랜덤 폴백이라 **안 채워도 안전하다**.
- **표시용 조회**: `GET /v1/shipping-countries` 목록이 요율표에서 파생한 `customerShippingKrw`(+`productCarrier`/`seedingCarrierSplitWeightG`)를 포함하므로 프론트가 고객 배송비를 그대로 미리보기하고, 미설정 국가는 선택지/미리보기에서 제외한다. 공개 `:iso2` 응답은 `exclusions` 를 포함(클라가 EFS 제외구역 차단을 미러). **구 `productLogisticsCostKrw` 는 응답에서 제거됐다.** 어드민 목록/상세(`/admin/shipping-countries`, `:iso2`)도 같은 `customerShippingKrw` 를 싣는다(파생 단일 출처 `withCustomerShipping`). ⚠️ **select 가 공개/어드민으로 갈린다**(2026-08-13) — `COUNTRY_SELECT`(공개) vs `ADMIN_COUNTRY_SELECT`(= 공개 + `seedingDeclaredValueUsd`)이고, 어드민 목록은 `listForAdmin()` 을 탄다. `list()` 를 공개(`/v1`)와 어드민이 **함께 쓰기 때문에** 신고가를 기본 select 에 넣으면 통관가가 그대로 공개된다. `onlyEnabled` 플래그에 얹지 않은 이유도 같다 — "어드민 = enabled 무관"이라는 우연한 결합이 나중에 깨지면 곧 유출이다.
...
- **관련 파일**: `shipping.service.ts`(`resolveProductShipping`/`resolveCarrier`/`pickCarrierForWeight`/`loadEnabledCountry`/`update`/exclusions CRUD/`matchExclusion`), `logistics-rate.service.ts`(요율표 — 시딩·일반주문 공용, 어드민 컨트롤러는 seeding 모듈), `shipping-rate.service.ts` + `admin-shipping-rate.controller.ts` + `xlsx-grid.ts`(시딩 EMS/DHL 비교요율 — 2026-07 seeding 모듈에서 이동, 엔드포인트 문서는 [seeding](./seeding.md). `xlsx-grid.ts` 는 **리더**(한컴셀 `x:` 접두사 파일 대응 자체 OOXML 파서)이고 **라이터**는 배송비용 export 와 공용인 `rate-matrix-xlsx.ts` 다 — 시트명 상수(`CARRIER_SHEET_NAMES`/`SEEDING_RATE_SHEET_NAME`)가 엑셀 왕복을 성립시키고, 리더는 그 목록으로 **탭을 잘못 고른 업로드를 거절**한다(`WrongSheetError`). 국가 상세 AI 임포트(`aiImportPreview`/`replaceTiers`)도 여기 있고 `rate-sheet-ai.service.ts` 를 배송비용과 공유한다 — ⚠️ 두 메서드 모두 조회·삭제 범위에 **carrier 가 반드시 포함**돼야 반대 캐리어 요율이 안 지워진다), 3 개 컨트롤러(admin country · admin shipping-rate · public country).
...
| Method | Path                                                       | 기능                                                |
|--------|------------------------------------------------------------|-----------------------------------------------------|
| GET    | `/admin/shipping-countries`                                | 국가 목록 (enabled 무관 전체 + exclusions 개수 + `customerShippingKrw` + `seedingDeclaredValueUsd`) |
| GET    | `/admin/shipping-countries/export`                         | 시딩 신고가 xlsx (`시딩신고가` 시트, 전 국가 1행씩 · 값은 달러). ⚠️ `:iso2` **앞**에 선언 |
| POST   | `/admin/shipping-countries/import/preview`                 | 신고가 xlsx 업로드 → 변경 diff (적용 안 함)          |
| POST   | `/admin/shipping-countries/import/apply`                   | 같은 파일 + `iso2List` → 선택분만 반영               |
| GET    | `/admin/shipping-countries/:iso2`                          | 특정 국가 상세 (+ exclusions 개수 + `customerShippingKrw`, 없으면 404) |
| PUT    | `/admin/shipping-countries/:iso2`                          | enabled · 제외구역 캐시 · **캐리어**(고정값·무게분기, 배송비용 탭) · efsBillingFeeKrw · currencyCode · seedingDeclaredValueUsd |
