...
- **정산탭과의 대조 (2026-09-10)**: 브랜드 정산탭이 같은 귀속(`Order.promotionId`)을 라인 배지 `{이름} 링크 유입` 으로 보여준다([settlement](./settlement.md)). ⚠️ **여기 구매수·매출과 그 화면의 정산액은 정확히 같은 수가 아니다** — 이쪽은 **주문 단위 · 전 기간 누적 · `Order.totalUsd` 전액**(배송비 포함, 멀티브랜드면 **남의 브랜드 몫까지**)이고, 정산탭 행은 **(주문 × 브랜드) = 송장 단위 · 그 달 원장 스코프 · PG 5% 차감 KRW** 다. 정산탭 툴팁이 이 차이를 명시한다. ⚠️⚠️ 또한 **`Order.promotionId` 는 브랜드 스코프가 아니다** — `resolvePromotionForOrder` 가 code 로만 조회하고 klow_web 이 localStorage 영속 code 를 카트 구성과 무관하게 싣기 때문에, A 브랜드 링크를 방문했던 손님이 B 브랜드 제품을 사면 그 주문이 A 링크에 귀속된다(여기 매출이 그만큼 과다 계상될 수 있다). 정산탭은 `Promotion.brandId === Shipment.brandId` 게이트로 **표시**만 막았고, **이 집계 자체는 아직 그대로다**(별도 과제).
...
- **클릭 집계 규칙 (2026-08-19 통일)**: 미존재(만료/오타) → 미집계. `enabled=false` 또는 `status!=='active'` → 미집계. 정상이면 `clickCount +1` + 오늘(KST) 일별 버킷 `clicks +1`. ⚠️ **호출자가 바뀌었다** — 예전엔 klow_web **서버 컴포넌트**가 `trackBySlug` 로 셌지만, 그 경로는 SNS 언펄러(봇)가 링크 하나당 여러 번 긁는 것을 전부 클릭으로 잡아 **할인 링크 성과를 부풀렸다**. 이제 klow_web **클라이언트 비콘**이 브랜드관 방문과 같은 요청으로 보내고 [storefront-stats](./storefront-stats.md) 가 `PromotionsService.recordClickByCode(code)` 를 부른다(카운터 소유권은 이 모듈에 남아 Off/중지 게이트가 한 곳이다). 결과적으로 **봇 제외 + 탭당 1회**라 전환 시 시계열이 한 번 계단처럼 내려가고, 애드블록 방문자는 안 잡힌다. 구 `trackBySlug` 는 집계를 뗀 **`resolveBySlug`**(세일가 code 해석 전용)가 됐고 라우트 경로의 `track` 은 klow_web 과의 계약이라 남은 legacy 다.
...
| Method | Path                                          | 기능                                                                            |
|--------|-----------------------------------------------|---------------------------------------------------------------------------------|
| GET    | `/v1/promotions/track/:brandSlug/:influencerSlug` | **세일가 code 해석 전용**(2026-08-19 부터 집계하지 않는다 — 경로 이름은 legacy). klow_web 서버 컴포넌트가 렌더 전에 호출. `{ ok, code }` 반환(무효/Off 는 `{ok:false, code:null}`). `code` 는 klow_web 이 localStorage 에 저장해 이후 가격/견적/주문에 동봉하는 **세일가 적용 키** |
