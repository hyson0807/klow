# promotions — 인플루언서 할인가 브랜드관 링크·유입 추적
...
> 안 바뀐 것: **공개 유입 링크 `klow.kr/{brand.slug}/{promotion.slug}`**(배포된 인플루언서 링크가 그대로 살아있다), `Promotion.code` 값, 저장된 데이터 전부.
...
- **모델 요약**: **인플루언서 1명 = 프로모션 1개 = 할인가 브랜드관 링크 1개.** 브랜드가 발급 마법사에서 이름 하나를 넣으면 곧 프로모션 1건이 생성된다(구 "프로모션 컨테이너 + 그 안에 링크 N개" 2단계 구조 폐기).
- **주 클라이언트**: `klow_brand`(자기 인플루언서 CRUD·발급·세일가 편집·유입 차트, `/v1/brand/promotions/*`) + `klow_admin`(전 브랜드 관찰·강제 On/Off·삭제, `/admin/promotions/*`) + 공개 트래픽(klow_web `klow.kr/{brand}/{influencer}` pretty 링크 클릭).
- **데이터 모델**: `Promotion`(brand 소유, `name`(인플루언서 표시명)/`slug`(브랜드 내 유니크, name 에서 자동 생성)/`code`(@unique 6자 영숫자, 유입 식별 키)/`clickCount`(누적)/`enabled`/`status`), **`PromotionProductPrice`**(`promotionId`×`productId`×`iso2` @unique, `priceLocal` Float non-null — 이 링크의 제품×국가 세일가), `PromotionDailyStat`(프로모션×날(KST) 1행, `date`(YYYY-MM-DD)/`clicks` — "기간별 유입 추이" 차트용). 삭제는 `onDelete: Cascade` 로 세일가·일별통계 함께 제거.
...
- **프로모션이란**: 브랜드가 **이름 + 제품별·국가별 판매가**를 입력해 발급하면, 서버가 `slug`·`code` 를 발급하고 공개 링크 `klow.kr/{brand.slug}/{promotion.slug}` 를 만든다. 인플루언서가 그 링크를 SNS 에 게시 → 유저 클릭 시 klow_web 이 유입 1 을 집계하고 **그 가격이 적용된 브랜드관**을 렌더한다(방문자는 pretty URL 에 머문다). 세일가는 비워도 발급되고(먼저 링크만 뽑는 흐름) 그때는 정상가 브랜드관으로 동작한다. **발급 후에도 상세 화면에서 언제든 고칠 수 있다.**
...
- **관련 파일**: `promotions.service.ts`(브랜드 CRUD·slug 생성·세일가 replace-all·유입 집계·일별 통계), 컨트롤러 3개(brand·admin·public), `promotions.module.ts`(`BrandAuthModule` import). klow_web `src/app/[brandSlug]/[influencer]/page.tsx`(pretty 랜딩) + `src/lib/brand-server.ts` `trackInfluencerVisit`. klow_brand `src/app/(authed)/promotions/`(`PromotionPriceEditor`/`PromotionPriceModal` + `_hooks/usePromotionProducts.ts`) + `src/lib/promotion-prices.ts`(맵↔배열 변환 단일 출처).
...
| Method | Path                              | 기능                                                                                       |
|--------|-----------------------------------|--------------------------------------------------------------------------------------------|
| GET    | `/v1/brand/promotions`             | 내 인플루언서 목록(각 item `id/name/status/clickCount/purchaseCount` — **per-item 매출·세일가는 안 실림**) + KPI(activePromotions/totalClicks/totalPurchases/totalRevenueUsd) |
| POST   | `/v1/brand/promotions`             | 인플루언서(=프로모션) 생성(`name` + `prices[]{productId,iso2,priceLocal}`, prices 는 생략 가능) — slug·code·url 발급 |
...
| Method | Path                   | 기능                                                                     |
|--------|------------------------|--------------------------------------------------------------------------|
| GET    | `/admin/promotions`     | 전체 목록(`q`=인플루언서명/브랜드명 검색, `status`, `take`/`skip`)         |
