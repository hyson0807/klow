...
`/v1` 밖의 **별도 prefix `/embed/*`** 입니다. prefix 하나 = 보안 자세 하나(전부 공개 · GET 전용 · 쿠키 없음 · CORS `*`)라 감사 지점이 단일해집니다. `/r/{code}`(promotions 단축링크)가 이미 `/v1` 밖 공개 라우트 선례입니다.
...
- 스크립트 소스는 `embed-script.ts` 의 **문자열 상수**다. ⚠️ raw `.js` 파일로 되돌리지 말 것 — 프로덕션에서만 404 가 났던 원인이다. (2026-09-07 청구서가 PDF 에서 엑셀로 바뀌며 번들 폰트와 함께 `nest-cli.json` 의 `compilerOptions.assets` 도 제거됐다 — **지금 dist 로 복사되는 에셋은 없다.** 이 스크립트를 raw `.js` 로 되돌리면 그 404 가 곧바로 재발한다. 문자열이면 압축본·ETag 를 모듈 로드 시 계산해 둘 수 있고 파일 I/O 도 없다.)
