...
- 스크립트 소스는 `embed-script.ts` 의 **문자열 상수**다. ⚠️ raw `.js` 파일로 되돌리지 말 것 — 프로덕션에서만 404 가 났던 원인이다. (2026-09 부터 `nest-cli.json` 에 `compilerOptions.assets` 가 생겨 `src/assets/**` 는 dist 로 복사되지만, 그건 **청구서 PDF 폰트 전용**이고 이 스크립트는 계속 TS 상수로 둔다 — 문자열이면 압축본·ETag 를 모듈 로드 시 계산해 둘 수 있고, 파일 I/O 도 없다.)
