...
- **⚠️ `Product.hidden` 은 온라인 전용 게이트다** *(2026-09-10)*: 브랜드 요청 — *"제품 가리기를 해도 현장모드에서는 안 가려지게"*. `hidden`("이 제품 가리기")은 **온라인 브랜드관에서 내린다**는 뜻이고, 부스에서 손님에게 직접 파는 축은 **별개 컬럼 `onsiteExcluded`** 가 담당한다. 두 축은 직교이며, 온라인에서 내린 제품도 부스에서는 계속 팔린다.
  ...
  - ⚠️ **온라인 축에는 절대 쓰지 말 것** — `/shop`·검색·서버 카트(`cart.service`)는 `PUBLIC_PRODUCT_WHERE` 그대로다. 쓰는 순간 브랜드가 내린 제품이 온라인에 되살아난다. (로그인 손님이 부스에서 담으면 `cart.service.upsert` 가 400 을 내지만 klow_web 이 `.catch(devWarn)` 로 삼키고 현장 결제는 로컬 카트를 직접 보내므로 구매는 정상이다 — 가려진 제품이 그 손님의 **온라인 카트에 남지 않는 게 오히려 맞다**.)
