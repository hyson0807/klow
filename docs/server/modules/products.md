...
- **공개 필터**: `PUBLIC_PRODUCT_WHERE`(= `PURCHASABLE_PRODUCT_WHERE`, 노출=구매 가능 시점이라 동일 조건). 게이트: `Product.status='approved'` + 대표사진(`image != ''`) + 가격 완성도 `hasSellablePrice`(`basePriceUsd>0` OR (`basePriceFxRate≠null` & `salePrice>0`), Prisma 짝은 `SELLABLE_PRICE_WHERE`) + **`Product.hidden=false`**(2026-07-03 추가, 브랜드가 스튜디오에서 직접 가린 제품 제외 — ⚠️ **현장(부스 QR)에는 적용되지 않는다**, 아래 `ONSITE_PRODUCT_WHERE`) + 브랜드 노출(가입 brand `submittedById != null` 는 `Brand.status='approved'` & 구독 active 동안만 / 어드민 생성 brand `submittedById = null` 은 `Brand.status='approved'` 만 / legacy `brandId = null` 은 면제).
- **⚠️ `Product.hidden` 은 온라인 전용 게이트다** *(2026-09-10)*: 브랜드 요청 — *"제품 가리기를 해도 현장모드에서는 안 가려지게"*. `hidden`("이 제품 가리기")은 **온라인 브랜드관에서 내린다**는 뜻이고, 부스에서 손님에게 직접 파는 축은 **별개 컬럼 `onsiteExcluded`** 가 담당한다. 두 축은 직교이며, 온라인에서 내린 제품도 부스에서는 계속 팔린다.
  - 게이트가 둘로 갈렸다: `PURCHASABLE_PRODUCT_WHERE`(= `PUBLIC_PRODUCT_WHERE`, `hidden:false` 포함) / **`ONSITE_PRODUCT_WHERE`**(= 같은 게이트에서 `hidden` 만 뺀 것). JS 짝은 `isPurchasable(p, { onsite })`.
  - ⚠️⚠️ **현장 경로 셋이 전부 같은 게이트를 써야 한다** — 브랜드관 목록(`findAll`, `onsiteMode`) · PDP(`findOne`, `opts.onsite`) · **주문 생성(`orders.createOnsite` 의 `isPurchasable(p, { onsite: true })`)**. 목록만 바꾸면 손님이 부스 화면에서 담을 수는 있는데 결제만 400 이 되어, **카드를 꺼낸 뒤에 막힌다**(부스 앞에서 가장 나쁜 실패이고, 타입도 응답 shape 도 그대로라 조용하다).
  - ⚠️ **온라인 축에는 절대 쓰지 말 것** — `/shop`·검색·자사몰 임베드(`embed.service`)·서버 카트(`cart.service`)는 `PUBLIC_PRODUCT_WHERE` 그대로다. 쓰는 순간 브랜드가 내린 제품이 온라인에 되살아난다. (로그인 손님이 부스에서 담으면 `cart.service.upsert` 가 400 을 내지만 klow_web 이 `.catch(devWarn)` 로 삼키고 현장 결제는 로컬 카트를 직접 보내므로 구매는 정상이다 — 가려진 제품이 그 손님의 **온라인 카트에 남지 않는 게 오히려 맞다**.)
  - ⚠️ `hidden` 만 면제된다. 미승인·미완성(대표사진/판매가)·브랜드 미승인·구독 만료는 **부스에서도 그대로 막힌다**(회귀 스펙이 각각 잠근다).
  - ⚠️⚠️ **파급: `hidden` 의 뜻이 좁아진다.** 부스 진입 URL `klow.kr/{slug}?mode=onsite` 는 **토큰 없는 공개 주소**라, 이 변경 이후 누구든 브랜드관 주소에 `?mode=onsite` 를 붙이면 가려진 제품을 볼 수 있다. `hidden` 은 이제 "고객에게 안 보임"이 아니라 **"기본 온라인 브랜드관에 안 보임"** 이다. 완전히 감추려면 현장 탭에서 `onsiteExcluded` 도 함께 꺼야 한다.
  - **klow_brand**: 현장 설정 보드(`OnsiteBoard`)가 `filter(!p.hidden)` 로 가려진 제품을 목록에서 빼고 있었다 — 그대로 두면 브랜드가 **온라인에서 내린 제품의 현장가를 정할 수 없어** 기준가 파생값으로 팔리거나 현장 제외도 못 한다. 제거했다(서버 `saveOnsiteConfig` 는 원래부터 `hidden` 을 안 봐 저장은 이미 열려 있었다). 스튜디오 가리기 안내에 "현장 QR 판매에는 영향이 없습니다" 를 명시한다.
  - **klow_admin**: 배지 `브랜드 가림 · 비노출` → **`브랜드 가림 · 온라인 비노출`**. 그냥 '비노출' 이면 운영자가 "아예 못 파는 제품"으로 읽는다.
...
- ⚠️ 읽기 경로는 캐시와 오버라이드를 **`Promise.all` 로 병렬 조회**한다(목록·PDP 핫 경로라 직렬로 이으면 왕복이 더해진다). 이 쿼리를 없애겠다고 **`Product` 에 마커 컬럼을 두면 안 된다** — `@updatedAt` 이 어떤 write 에도 반응해 7개 로케일 캐시가 전부 무효화되고 재번역 폭풍이 난다. **오버라이드 읽기·쓰기 어느 쪽도 `Product` 를 쓰지 않는다**(스펙이 단언).
