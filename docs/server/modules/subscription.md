...
- **금액·주기**: `billingInterval` = `monthly` / `annual`. 월 기본 ₩49,500(env `SUBSCRIPTION_MONTHLY_PRICE_KRW`), 연 기본 ₩501,600(env `SUBSCRIPTION_ANNUAL_PRICE_KRW`). env 미설정/0/비정상이면 경고 로그 + 기본값 폴백. 다음 결제일 = `monthly`→+1개월 / `annual`→+12개월. 일시불(`cardQuota=0`) 고정.
