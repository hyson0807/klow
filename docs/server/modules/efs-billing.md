...
**`Remarks (REF #)` 열 = 현지 송장번호** *(2026-09)*. `settlementRefColumn` 이 헤더 정규식으로
결정적으로 찾고(관세 귀속 조인에도 같은 열을 쓴다), `importApply` 가 **`Shipment.localTrackingNumber`
에 채운다** — 청구를 확정하는 그 순간에 함께 넣으므로 EFS API 를 따로 왕복하지 않는다.
- ⚠️ **저장은 원문 그대로**(trim + 40자 절단)다. 이 값이 고객 `/track` 의 캐리어 판별·조회 URL
  입력이 되기 때문이다 — 대문자화·하이픈 제거(`normalizeRegistryNo`)는 **관세 조인 키에만** 쓴다.
- ⚠️ REF 칸에 HAWB 를 그대로 적어 둔 행(실측 118건 중 3건)은 **저장하지 않는다** — 조인에도
  표시에도 쓸모가 없고, 넣으면 '현지 번호가 있다'고 거짓말하게 된다.
- ⚠️ 재업로드로 이 값을 채우려면 `CompareModal` 의 `sideRows`(금액은 같은데 부수 값만 바뀌는 행)에
  **현지 송장번호 비교가 들어 있어야 한다** — 실비를 이미 저장해 둔 달은 그게 유일한 경로다
  (청구중량·수출신고번호가 같은 이유로 거기 들어 있다).
...
> **주문번호 → 송장번호·서비스 (2026-09)** — 서비스 타입은 **EFS 회신 원문**(EMS·PREMIUM·
> EMSPREMIUM·EMSDTP·DHL·USCOS…)을 그대로 싣는다(우리가 이름을 지어내면 EFS 문서·추적 화면과 갈린다).
> ⚠️ `serviceType` 은 원래부터 동결 스냅샷에 있던 키라 과거 확정본도 그대로 채워지고, nullable 이라
> `—` 폴백이다.
>
> ⚠️⚠️ **'송장번호' 는 HAWB 가 아니라 현지(라스트마일) 송장번호**(`Shipment.localTrackingNumber`
> = 정산표 `Remarks (REF #)`)다 — `EG050851717KR`·`UP900986735KR`·`GFUS01070348013060` 처럼
> **수취인이 실제로 조회할 수 있는 번호**여야 브랜드가 대조한다. HAWB(`EFS1005195591`)는 EFS
> 내부 번호라 어디서도 조회되지 않는다.
> - **없으면 HAWB 로 폴백한다.** 청구서 행에 식별자가 아예 없으면 브랜드는 그 건이 무엇인지
>   물어볼 수밖에 없다(수기 청구 행이 여기 해당한다 — `EfsManualBillingRow` 엔 현지 번호가 없다).
> - ⚠️ `localTrackingNumber` 는 **`EfsStatementRow` 에 2026-09 에 추가된 키**라 그 이전 동결
>   스냅샷엔 없다 → 과거 확정본은 전부 HAWB 폴백으로 렌더된다(백필 불가, 마이그레이션 없음).
> - 값을 채우는 주 경로는 **정산표 업로드**다(아래 파싱 절). 실측 2026-08 파일 기준 118건 중
>   **115건(97%)** 에 현지 번호가 있고, 3건은 REF 칸에 HAWB 가 적혀 있어 폴백된다.
