...
- **설계 정본**: [`../../plan/custom-domain/implementation-plan.md`](../../plan/custom-domain/implementation-plan.md) §2 (P1). 배경은 [`../../plan/custom-domain/flow.md`](../../plan/custom-domain/flow.md), 결정표는 [`../../plan/custom-domain/README.md`](../../plan/custom-domain/README.md).
...
> **설계 정본**: [`../../plan/custom-domain/purchase-plan.md`](../../plan/custom-domain/purchase-plan.md).
...
| **만료일은 `registrations/{domain}` 에서만 읽는다** | `registration-status` 는 진행 `state` 만 주고 `expires_at` 을 **절대 싣지 않는다**(§0 2번 실측). 거기서 읽으면 ① 모든 구매가 근사 만료일로 시작해 갱신 대상에서 빠지고(1년 뒤 소멸) ② §9-3 의 전진 비교가 영원히 거짓이라 정상 갱신이 전부 `action_required` 로 떨어진다. ⚠️ 그렇다고 `registration-status` 를 대체하지도 말 것 — 상태 머신이 쓰는 `failed`·`action_required`·`blocked` 가 그쪽엔 없다. ⚠️ 근사값 행은 `handleEstimated` 가 벤더에 다시 물어 **자가치유**한다(사람 손에 맡기면 아무도 안 채우고 도메인이 만료된다) |
| **되조회에서 `status`가 `approved`일 때만 `paid` 로 확정한다** | 4단계 타임아웃은 `netCancel` 을 먼저 부르므로 되조회에서 가장 흔한 상태가 **취소된 거래**다. status 를 안 보면 브랜드는 한 푼도 안 냈는데 우리는 환불 불가능한 도메인 원가를 문다. `void`(취소·만료·거절)는 `failed` 확정, **모르는 값·부분취소는 `action_required`**(모르는 값을 `void` 로 폴백하면 승인된 결제를 미승인으로 확정한다) |
| **금액을 못 읽은 것도 불일치와 같은 칸이다** | `amountKrw === null` 을 통과시키면 금액 검증이 통째로 사라진 채 `paid` 가 된다(Eximbay 의 `parseFloat("132,000")` 선례) |
| **타임아웃이면 갱신 청구도 `netCancel` 을 부른다** | 구매·구독은 부르는데 갱신만 빠져 있었다. 이 배치는 하루 1회라 안 부르면 NicePay 망취소 창(1시간)이 지나가고, 브랜드 카드는 긁힌 채 우리 장부만 `pending` 으로 남는다 |
