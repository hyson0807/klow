...
- **payload 빌드 (`payload-builder.ts`)**: `buildPayload` 가 33 필드를 `|` join 한 `sendData` 를 만든다. 핵심:
  ...
  - **캐리어 → EFS 서비스 타입**: EFS→`Premium` / EMS→`EMS` / **EMS_PREMIUM→`EMSPREMIUM`** / DHL→`DHL` (`CARRIER_TO_EFS_SERVICE_TYPE`, `Record<ShippingCarrier, string>` 이라 enum 추가 시 컴파일이 누락을 잡는다). 캐리어는 **외부 API 라우팅이 아니라 이 3번 필드 문자열 하나로만 번역**된다 — 전 캐리어가 같은 `EfsClient.newCreateShipment` 로 나간다. ⚠️ **이 문자열은 EFS 가 정한 토큰이라 추측하면 안 된다** — 모르는 값이면 EFS 가 `Service type is invalid` 로 **발급 자체를 거절**한다(필드 위치·개수가 맞아도 소용없다). EMS_PREMIUM 은 `EMS Premium`(공백 有)으로 보냈다가 실제로 거절당했고, EFS 확인 회신으로 **공백 없는 대문자 `EMSPREMIUM`** 이 정본임을 받았다(2026-08). `Shipment.efsServiceType` 은 `VarChar(20)` — 현재 최장값 `EMSPREMIUM` 은 10자라 구 `VarChar(10)` 에도 맞지만, 새 서비스 타입이 조용히 잘리지 않도록 여유를 유지한다. 캐리어 값 자체는 주문 시점 스냅샷 `Order.shippingCarrierByBrand[brandId]` 우선, 없으면 대표 `Order.shippingCarrier` 폴백(`carrierForBrand`).
