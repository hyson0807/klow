...
- **payload 빌드 (`payload-builder.ts`)**: `buildPayload` 가 33 필드를 `|` join 한 `sendData` 를 만든다. 핵심:
  ...
  - **캐리어 → EFS 서비스 타입**: EFS→`Premium` / EMS→`EMS` / **EMS_PREMIUM→`EMS Premium`** / DHL→`DHL` (`CARRIER_TO_EFS_SERVICE_TYPE`, `Record<ShippingCarrier, string>` 이라 enum 추가 시 컴파일이 누락을 잡는다). 캐리어는 **외부 API 라우팅이 아니라 이 3번 필드 문자열 하나로만 번역**된다 — 전 캐리어가 같은 `EfsClient.newCreateShipment` 로 나간다. `'EMS Premium'` 이 11자라 `Shipment.efsServiceType` 은 `VarChar(20)`(2026-08 확장, 구 `VarChar(10)`). 캐리어 값 자체는 주문 시점 스냅샷 `Order.shippingCarrierByBrand[brandId]` 우선, 없으면 대표 `Order.shippingCarrier` 폴백(`carrierForBrand`).
