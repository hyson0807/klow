...
| `/shop`·검색 유입 | 제외 | 포함 |
| 브랜드 상품 링크(`?brand=`) | **포함**(2026-08-20~) | 포함 |
...
브랜드 피드백 — "브랜드관으로 들어가야만 집계된다". 브랜드가 자사몰·SNS 에 붙인
`/product/{id}?brand={slug}` 링크 유입이 방문뿐 아니라 **담기·결제까지** 통째로 빠지고 있었다
...
- ⚠️ 브랜드관 그리드의 상품 링크에도 `?brand=` 가 붙어 **외부 딥링크와 형태가 같다**(구별 불가).
...
- ⚠️⚠️ **국가 dedupe 키를 `productId` 로 되돌리지 말 것.** 그 축이면 `trackStorefrontProductVisit`
  ...
  않는다**(2026-08-20~21 실제 버그 — 딥링크 유입 국가가 100% '미상'이었다). 축은 브랜드
...
- 이게 "브랜드를 거친 사람이 담는가"라는 정의 그 자체다. `/shop`·검색에서 담은 건은 방문
  모집단 밖이라 빠진다.
- ⭐ **딥링크 PDP 는 2026-08-20 부터 빠지지 않는다** — PDP 방문이 원장 행을 만들면서
...
⚠️ **딥링크 PDP 는 2026-08-20 부터 진입 시점에 이미 잡힌다**(그전엔 브랜드관 `/{slug}` 을
거쳤을 때만 잡혔다).

⚠️ **딥링크 PDP 를 먼저 본 사람은 그날 귀속이 `direct` 로 고정된다.** 오전에 딥링크 PDP 로 들어와
...
밖이고, 이틀 이상 지난 방문의 결제도 빠진다(딥링크 PDP 는 2026-08-20 부터 포함). **정산·주문 화면의 숫자와 다른 게 정상**
이며, 그쪽이 매출의 정본이다.
...
- ⚠️ **`sentCountries` 의 축은 "브랜드 문맥 × 국가"다 — 제품이 아니다.** 원장 키가
  ...
  **탭 × 브랜드 하나**다. 그래서 키가 두 별칭(`id:{brandId}:{cc}` / `slug:{slug}:{cc}`)이고
  ...
  국가를 나중에 고른 딥링크 PDP 손님(= 이 기능의 주 대상)의 국가가
...
⚠️ **배포 첫날 `uniqueVisits(direct)` 와 `purchases` 가 계단처럼 올라간다**(딥링크 유입이 새로
