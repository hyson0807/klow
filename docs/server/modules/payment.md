...
- ⚠️⚠️ **`req.ip` 가 호출자 IP 인지부터 확인해야 한다.** 2026-09-04 까지 `main.ts` 의 `trust proxy` 가 `1` 로
  잡혀 있어 `req.ip` 에 **호출자가 아니라 우리 앞단 Railway 엣지 IP**(`152.233.x.x`, CDN77-SGP)가 들어왔고,
  그래서 화이트리스트에 무엇을 넣든 **구조적으로 매칭이 불가능**했다(= 모든 결제 콜백 403).
  ✅ 2026-09-05 에 `TRUST_PROXY_HOPS=2` 로 고치고 운영에서 검증했다 — 6패턴 모두 `req.ip` 가 호출자 IP 였고
  XFF 에 화이트리스트 값을 채워 넣는 우회도 막혔다. 재검증 방법은 같다: 화이트리스트에 없는 IP 에서 이
  엔드포인트를 호출해 403 로그의 IP 가 호출자 공인 IP 와 같은지 본다.
  ⚠️⚠️ **`152.233.14.0/23` · `152.233.68.0/23` 을 화이트리스트에 넣지 말 것** — Eximbay 대역이 아니라 Railway
  엣지라, 넣으면 Railway 를 통과한 모든 요청이 매칭되어 화이트리스트가 통째로 무력화된다(실제로 한 번 그렇게
  넣어 엔드포인트가 인터넷에 열렸다).
- **Eximbay 운영 발신 IP 는 `15.165.144.33` 하나다**(2026-09-05 실측 확인 — 공식 문서 값과 일치).
  문서는 `172.28.11.71` / `172.28.11.72` 도 함께 안내하지만 그 둘은 **사설 대역**(172.16.0.0/12)이라 외부에서
  출발지로 나타날 수 없고, 실제 관측되지도 않았다. 샌드박스는 `43.203.92.211,3.34.20.184,3.37.76.229,52.79.143.149`.
  ⚠️ Eximbay 가 발신 서버를 늘리면 그 IP 는 403 이 나므로, 거부 로그(`eximbay webhook from non-whitelisted ip <IP>`)에
  새 IP 가 보이면 추가한다. 통과분은 `eximbay webhook accepted from <IP>` 로 남는다.
