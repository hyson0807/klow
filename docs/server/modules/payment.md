...
- ⚠️⚠️ **`req.ip` 가 호출자 IP 인지부터 확인해야 한다.** 2026-09-04 까지 `main.ts` 의 `trust proxy` 가 `1` 로
  ...
  넣어 엔드포인트가 인터넷에 열렸다).
- ⚠️ **Eximbay 운영 발신 IP 는 아직 확정되지 않았다.** 공식 문서(개발자센터 웹훅 페이지)는 `172.28.11.71` /
  `172.28.11.72` / `15.165.144.33` 을 안내하는데 앞의 둘은 **사설 대역**(172.16.0.0/12)이라 그대로는 매칭될 수
  없다. 정본은 로그에 찍히는 실제 IP 다 — 거부 시 `eximbay webhook from non-whitelisted ip <IP>`,
  통과 시 `eximbay webhook accepted from <IP>`.
