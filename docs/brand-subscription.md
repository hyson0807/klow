...
**단일 플랜 (`brand_standard`) · 신규 가입은 6개월/연 주기만 선택.**

| 주기 | 공급가(KRW) | 실청구액(VAT 10% 포함) | 월 환산 |
|---|---|---|---|
| 6개월납 (`semiannual`) | ₩300,000 | **₩330,000** | ₩55,000 |
| 연납 (`annual`)  | ₩480,000 | **₩528,000** | ₩44,000 |
| 월납 (`monthly`, **legacy**) | ₩45,000 | **₩49,500** | ₩49,500 |

- **무료 체험 없음** — 심사 정보 제출 직후 즉시 결제.
- 결제 주기(`BrandSubscription.billingInterval`)가 금액과 다음 결제일(6개월 +6개월 / 연 +12개월 / 월 +1개월)을 가른다.
- **`monthly` 는 legacy 전용**: production 의 기존 월결제 구독을 위해 백엔드에 유지. 신규 가입은 못 고르지만(validation 차단), 저장된 `monthly` 는 `normalizeInterval` 이 그대로 보존해 **매월 49,500원으로 계속 청구**된다(6개월으로 정규화되지 않음). interval 을 사용자 입력으로 쓰는 곳은 `start`(신규/재가입)뿐이고 `changeCard`/`resume`/cron 은 저장값을 보존하므로, 기존 월결제 브랜드는 카드 변경·재개를 해도 월결제로 남는다.
- 단일 플랜이라 `SubscriptionPlan` 테이블을 만들지 않고 **코드 상수 + 환경변수**로 관리한다:
  - `SUBSCRIPTION_MONTHLY_PRICE_KRW=49500` (legacy 월결제 구독 청구액)
  - `SUBSCRIPTION_SEMIANNUAL_PRICE_KRW=330000` (미설정·비정상 시 코드 기본값 330000 + WARN 로그)
  - `SUBSCRIPTION_ANNUAL_PRICE_KRW=528000`
...
model BrandSubscription {
  ...
  billingInterval    String   @default("monthly")   // 신규 'semiannual' | 'annual', legacy 'monthly' 보존
...
SUBSCRIPTION_MONTHLY_PRICE_KRW=49500      # legacy 월결제 구독 전용 (신규 가입 불가)
SUBSCRIPTION_SEMIANNUAL_PRICE_KRW=330000
SUBSCRIPTION_ANNUAL_PRICE_KRW=528000
SUBSCRIPTION_PLAN_CODE=brand_standard
SUBSCRIPTION_ORDER_NAME_PREFIX=KLOW 브랜드 멤버십
```
...
- **가입 진입 흐름** (`studio/page.tsx` 오케스트레이션):
  ...
  2. `PaymentWallPanel` 의 `needsPaymentWall(brand)` = `needsOnboarding && !!pgCustomerKey` → 같은 탭이 자동으로
     paywall 로 전환. KLOW PRO 플랜(6개월 월환산 ₩50,000 / 연 월환산 ₩40,000, `billingInterval` 토글) + 자동결제 동의
