...
>
> 본 문서는 결정이 굳어질 때마다 섹션을 보강해나가는 **살아있는 설계 문서**다. NHN KCP 자동결제(배치키) + 단일 플랜(₩35,000/월) + 입점 신청과 결제의 결합으로 운영된다.
...
- **단일 플랜이므로 `SubscriptionPlan` 테이블을 생성하지 않는다.** 코드 상수 + 환경변수로 시작:
  - `SUBSCRIPTION_MONTHLY_PRICE_KRW=35000`
...
| 메서드·경로 | 역할 | 상태 |
|---|---|---|
| `POST /start` | 토스 `authKey` → 빌링키 교환 + 최초 결제(₩35,000) + `approveApplication()` 자동 호출. 신규/재가입 처리 | ✅ |
...
- `POST /v1/billing/{billingKey}` ({customerKey, amount: 35000, orderId, orderName}) — 자동결제 승인
...
- 단일 ₩35,000 플랜이라 일할 정산·세금 계산 부담 대비 가치 낮음
...
SUBSCRIPTION_MONTHLY_PRICE_KRW=35000                 # 미설정 시 코드 기본값 35000
SUBSCRIPTION_PLAN_CODE=brand_standard
SUBSCRIPTION_ORDER_NAME_PREFIX=KLOW 브랜드 멤버십
```
...
- [x] PG 확정 (토스페이먼츠 빌링키)
- [x] 요금제 구조·가격·플랜 수 확정 (단일, 월 ₩35,000)
- [x] 무료 체험 정책 (없음)
