...
> **상태:** **Phase 1 (결합 트랜잭션) 구현 완료** — `POST /v1/brand/subscription/start` 가 빌링키 발급 + 첫 결제 + 자동 입점 승인까지 한 번에 처리. Phase 2~5 (노출 게이트 적용 / 라이프사이클 운영 / 정기 청구 / webhook · 운영 도구) 는 후속 작업으로 남음.
...
> 기존 KLOW 는 "노출(공개 surface)은 rejected 만 차단, 구매(`isPurchasable`)는 approved-only" 의 **분리 정책** 이었음 (`CLAUDE.md` + `docs/architecture.md` Brand 입점 단락 참고).
>
> 본 문서 도입과 함께 **노출·판매 모두 "구독 active" 단일 게이트로 통일** 한다. `PUBLIC_PRODUCT_WHERE` / `PURCHASABLE_PRODUCT_WHERE` 가드 추가 + `docs/architecture.md` Brand 입점 워크플로우 단락과 `CLAUDE.md` 동일 단락 동기화는 [Phase 2](#phase-2--노출판매-게이트-적용) 에서 처리.
...
- `PUBLIC_PRODUCT_WHERE` 에 `brand: { subscription: { is: { status: 'active' } } }` 가드 추가
- `PURCHASABLE_PRODUCT_WHERE` 에 동일 가드 추가
- 결과: **결제 전 brand (`subscription=null`) 와 연체 brand (`status='past_due'`) 모두 노출·판매 차단**. 활성 결제 brand 만 통과.
...
└── subscription/             ← (신규)
    ...
    ├── subscription.service.ts            // 도메인 로직 (start/cancel/get/change-card/dunning)
    ├── billing-key.service.ts             // 빌링키 발급/교체/삭제
    ├── invoice.service.ts                 // 청구서 생성, dunning 재시도 큐
    ├── providers/
    │   ├── toss-billing.adapter.ts        // 토스 자동결제 API 래퍼
    │   └── billing-provider.interface.ts
    ├── controllers/
    │   ├── brand-subscription.controller.ts   // @Controller('v1/brand/subscription') + BrandGuard
    │   └── admin-subscription.controller.ts   // @Controller('admin/subscriptions') + AdminGuard
    ├── webhooks/
    │   └── toss-subscription-webhook.controller.ts // @Controller('webhooks/subscription/toss')
    └── cron/
        └── subscription-billing.cron.ts   // 일일 정기 청구 + dunning 재시도
```
...
| 메서드·경로 | 가드 | 역할 |
|---|---|---|
| `POST /v1/brand/subscription/start` | BrandGuard | **★ 핵심.** 토스 `authKey` → 빌링키 교환 + 최초 결제(₩30,000) + `approveApplication()` 자동 호출. 신규 결제와 재가입 모두 처리 |
| `GET /v1/brand/subscription` | BrandGuard | 현재 상태·다음 결제일·카드 정보·청구서 요약 |
| `POST /v1/brand/subscription/cancel` | BrandGuard | `cancelAtPeriodEnd=true` 설정 (현재 사이클 끝까지 사용) |
| `POST /v1/brand/subscription/change-card` | BrandGuard | 카드 교체. 토스 `authKey` → 새 빌링키 발급 → `BillingKey` row 교체 → 기존 키 soft delete + 토스 빌링키 삭제 API. 결제는 발생 안 함 |
| `GET /v1/brand/subscription/invoices` | BrandGuard | 청구서 히스토리 |
| `POST /webhooks/subscription/toss` | none | 토스 webhook 수신 (멱등 + 토스 조회 API 재검증) |
| `GET /admin/subscriptions` | AdminGuard | 운영 도구 (Phase 5) |
| `PATCH /admin/subscriptions/:id/refund` | AdminGuard | 예외적 환불 (운영 도구, 분쟁 대응용) |
...
### 정기 결제 (2회차 이후) 실패 → dunning

- **재시도 스케줄: 0일 / 1일 / 3일 / 7일** (최대 4회)
- **403 계열만 자동 재시도.** 400 계열은 즉시 `BrandSubscription.status='past_due'` 로 전이하고 카드 갱신 안내 발송 (재시도해도 같은 에러).
- 4회 모두 실패 → `status='past_due'` 확정 + 운영자 알림. 노출/판매는 게이트가 자동으로 차단 (별도 status 조작 없음).
- 사용자가 `POST /v1/brand/subscription/change-card` 로 카드 교체하면 다음 cron 사이클에서 새 카드로 재시도. 성공 시 `past_due` → `active` 전이.
...
- 엔드포인트: `PATCH /admin/subscriptions/:id/refund` (AdminGuard + AdminAudit)
- 내부적으로 토스 `POST /v1/payments/{paymentKey}/cancel` 호출
- body: `{ cancelReason: string, cancelAmount?: number }` — `cancelAmount` 생략 시 전액 환불 (단일 플랜이라 사실상 전액)
- 성공 시 단일 트랜잭션:
...
- `/studio/billing` — 현재 구독·다음 결제일·청구서 목록
- `/studio/billing/payment-methods` — 카드 등록·변경
- `/studio/billing/cancel` — 해지 (현재 사이클 끝까지 사용 안내 명시)

### 클라이언트 헬퍼

`klow_brand/src/lib/api.ts` 에 `subscription.*` namespace 추가:

- `subscription.start({ authKey, customerKey })` — 신규/재가입
- `subscription.get()` — 현재 상태
- `subscription.cancel()` — 사이클 만료 후 해지
- `subscription.changeCard({ authKey, customerKey })` — 카드 교체
- `subscription.listInvoices()` — 청구서 히스토리
...
### Phase 2 — 노출/판매 게이트 적용

- [ ] `PUBLIC_PRODUCT_WHERE` / `PURCHASABLE_PRODUCT_WHERE` 에 `subscription.status='active'` 조건 추가 (참고: 두 상수는 현재 동일 alias → 한 곳만 수정하면 됨)
- [ ] `docs/architecture.md` + `CLAUDE.md` Brand 입점 단락 동기화
- [ ] 영향 surface(`products`/`discover`/`shop`/`creators`/`videos`/`cart`) 회귀 테스트

### Phase 3 — 라이프사이클 운영

- [ ] `GET /v1/brand/subscription` + `POST /v1/brand/subscription/cancel`
- [ ] `POST /v1/brand/subscription/change-card`
- [ ] `/studio/billing` 대시보드 메뉴

### Phase 4 — 정기 청구 & dunning

- [ ] 일일 cron — `currentPeriodEnd` 도래 brand 일괄 결제 (`subscription-billing.cron.ts`)
- [ ] dunning 재시도 (0/1/3/7일, 403 계열만)
- [ ] `BrandSubscription.status='past_due'` 전이 + 사용자 배너 + 운영자 알림

### Phase 5 — Webhook & 운영 도구

- [ ] `POST /webhooks/subscription/toss` (멱등 + 토스 조회 API 재검증) — Stage 2↔3 사고 복구 채널 겸용
- [ ] `klow_admin` 구독·청구서 뷰, 예외 환불 도구
...
- [ ] `BrandSubscription.status='past_due'` 진입 시 사용자 알림 채널 (인앱 배너 외에 이메일/SMS 추가 여부)
- [ ] `approveApplication()` 의 actor 확장 후 어드민 audit log 에 `system:subscription` 액션 표기 형식
