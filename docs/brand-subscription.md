...
>
> 본 문서는 결정이 굳어질 때마다 섹션을 보강해나가는 **살아있는 설계 문서**다. NHN KCP 자동결제(배치키) + 단일 플랜(월 ₩40,000 / 연 ₩420,000) + 입점 신청과 결제의 결합으로 운영된다.
...
- **단일 플랜 / 월 ₩40,000 · 연 ₩420,000 (KRW) / VAT 포함 (확정)** — 결제 주기(월/연)만 선택, 연납은 월 환산 ₩35,000(연 ₩60,000 절약)
- **무료 체험 없음** — 가입·심사 직후 즉시 결제
- **결제 주기:** 매월(또는 매년), 최초 결제일 기준 같은 날 (예: 1/15 첫 결제 → 2/15, 3/15 …)
- **단일 플랜이므로 `SubscriptionPlan` 테이블을 생성하지 않는다.** 코드 상수 + 환경변수로 시작:
  - `SUBSCRIPTION_MONTHLY_PRICE_KRW=40000`
  - `SUBSCRIPTION_ANNUAL_PRICE_KRW=420000`
...
| 메서드·경로 | 역할 | 상태 |
|---|---|---|
| `POST /start` | 토스 `authKey` → 빌링키 교환 + 최초 결제(월 ₩40,000 / 연 ₩420,000) + `approveApplication()` 자동 호출. 신규/재가입 처리 | ✅ |
...
- `POST /v1/billing/{billingKey}` ({customerKey, amount: 40000, orderId, orderName}) — 자동결제 승인 (연납은 420000)
...
- 단일 월 ₩40,000 플랜이라 일할 정산·세금 계산 부담 대비 가치 낮음
...
SUBSCRIPTION_MONTHLY_PRICE_KRW=40000                 # 미설정 시 코드 기본값 40000
SUBSCRIPTION_ANNUAL_PRICE_KRW=420000                 # 연납 (미설정 시 코드 기본값 420000)
SUBSCRIPTION_PLAN_CODE=brand_standard
SUBSCRIPTION_ORDER_NAME_PREFIX=KLOW 브랜드 멤버십
```
...
- [x] PG 확정 (토스페이먼츠 빌링키)
- [x] 요금제 구조·가격·플랜 수 확정 (단일, 월 ₩40,000 / 연 ₩420,000)
- [x] 무료 체험 정책 (없음)
