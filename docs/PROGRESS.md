...
명세는 트랙 문서 §8 의 4-1 표 행 + §4-C·§4-D·§4-E·§4-G · §5-B, 그리고 **§2-C(주문 API 실측)**.
`import`(기간 14일·5페이지 상한 · 미러 upsert · 잘림 처리 · 재수집 정책) · `GET /orders` ·
`cafe24-retention.cron.ts`(**cron 11 → 12**).

✅ **막힘이 풀렸다.** 테스트 몰 `simsgood1` 에 주문 1건이 생겨 §2 B·F 를 채웠다(§2-C).
연결도 살아 있어 터널·재동의 없이 API 를 부를 수 있다.

⚠️⚠️ **착수 시 §2-C 의 함정 둘을 먼저 읽을 것**

1. **상태 정본은 `order_status`** 다. `status_code` 는 상태가 바뀌어도 `"N1"` 그대로라
   **판정에 쓰면 미결제 주문이 전환 대상에 섞인다.** 주문 최상위에는 `order_status` 가 **없다**
2. **매핑 키는 실제 `variant_code`** 다(빈 문자열 아님). 옵션 없는 상품도 실값을 싣는다

⚠️ 관측된 상태 코드가 **둘뿐**(`N00` 입금전 / `N20` 배송준비중)이라 **기본 필터를 정하지 않는다**
— 기간으로만 불러오고 `order_status`·`status_text` 를 미러에 저장해 화면이 고르게 한다.
나머지 코드를 추측해 상수로 박지 말 것.
