...
**cafe24-fulfillment 4-0단계 — 출고신청 목록 서버 기간 필터 (R3)** (2026-09-23 기록)

명세는 트랙 문서 §8 의 4-0. **자격증명 없이 끝까지 간다.**
...
| 실행 | 단계 | 한 줄 |
|---|---|---|
| **1** | **4-0** 출고신청 목록 기간 필터 | 3PL 부채(R3). **4-1 유입 전에** 끝나 있어야 한다 |
...
#### 4-0. 서버+브랜드 — 출고신청 목록 기간 필터 (R3) · **지금 할 것**

본문은 트랙 문서 [§8 의 4-0](./plan/cafe24-fulfillment/implementation-plan.md) 에 있다.

- **읽을 것**: 트랙 문서 `§8 의 4-0`·`§7 R3`, `server/modules/fulfillment.md` 의 브랜드 목록 절,
  `RequestsPanel.tsx` 머리말 주석
- **건드리는 레포 · 배포 순서**: **klow_server → klow_brand**(뒤집으면 **조용한 strip** — 구 서버가
  모르는 키를 버려서 기간이 무시된 최신 200건이 온다)
- **스키마·데이터 위험**: **없음** (⚠️ 인덱스도 추가하지 않는다 — 트랙 문서에 근거)
- **완료 기준**: 트랙 문서 §8 4-0 의 4줄

#### 5-1. 브랜드 화면 — 연동 (자격증명 무관 · **4-2 보다 먼저**)
...
| 9 | cafe24-fulfillment | **4-0. 출고신청 목록 기간 필터 (R3 · 자격증명 무관)** | 대기 | ✗ | | |
