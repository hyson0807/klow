...
#### 4-1. 서버 — 주문 불러오기 · **지금 할 것**

본문은 트랙 문서 [§8 의 4-1](./plan/cafe24-fulfillment/implementation-plan.md) 표 행 +
`§4-C`·`§4-D`·`§4-E`·`§4-G`·`§5-B`. 여기엔 읽을 것과 완료 기준만 둔다.

- ✅ **선행 조건 충족** — 연결이 살아 있고(테스트 몰 `simsgood1`), §2 B·F 가 **§2-C** 에 채워졌다
- **읽을 것**: 트랙 문서 **`§2-C`(주문 API 실측 — 함정 둘)**, `§4-C`·`§4-D`·`§4-E`·`§4-G`,
  `§5-B`, `§7 R4·R5·R7·R8`, `server/modules/cafe24.md`
- **건드리는 레포 · 배포 순서**: klow_server 만. **배포 없음**(6단계에서 함께 나간다)
- **스키마·데이터 위험**: **없음** (1단계에서 끝났다 — `Cafe24Order`·`Cafe24OrderItem` 이 이미 있다)
- **할 일**: `POST /orders/import`(기간 **14일**·**5페이지** 상한 · 미러 upsert · §4-E 잘림 처리 ·
  §4-G 재수집 정책) · `GET /orders`(**기본 필터 = 미전환**, R7) · `cafe24-retention.cron.ts`(R4)
- **완료 기준**
  - 실제 몰에서 **불러오기 1회** → `{fetched, inserted, updated, skipped, hasMore}` 확인
  - **재수집이 멱등** — 같은 기간을 두 번 불러도 미러가 늘지 않고 `updated` 로 잡힌다
  - ⚠️ **전환된 미러는 재수집이 덮어쓰지 않는다**(§4-G) — 덮으면 파기한 PII 가 되살아난다
  - **cron 11 → 12**: `test/app.e2e-spec.ts` 의 개수·이름 목록을 함께 고치고 통과
  - `docs/server/modules/cafe24.md` **갱신**
- ⚠️⚠️ **§2-C 의 함정 둘** — 상태 정본은 `order_status`(`status_code` 는 상태가 바뀌어도 `"N1"`
  그대로라 쓰면 **미결제 주문이 섞인다**) · 주문 최상위에 `order_status` 가 **없다**
- ⚠️ **기본 상태 필터를 정하지 않는다** — 관측 코드가 둘뿐(`N00`·`N20`)이다. 기간으로만 불러오고
  `order_status`·`status_text` 를 미러에 저장해 화면이 고르게 한다
...
| # | 단계 | 한 줄 |
|---|---|---|
| 4-2 | 서버 — 전환 | ⚠️ **G4 락 순서**(미러 → 재고) · §5-D 수량 합산 · 역기록이 재고 차감과 같은 트랜잭션 · 전환 후 PII 파기 |
| 5-2 | 브랜드 화면 — 매핑 | ⚠️ 가장 어렵다. 폭 ~532px · 행당 select + 검색 + 페이지네이션. **독립 페이지가 될 수도 있다** |
| 5-3 | 브랜드 화면 — 주문 목록 + 전환 | 불러오기 모달 · 프리뷰 차단 사유 · 유입 배지 · R7 안내 |
| 6 | **운영 배포 (3PL + 카페24 한 번에)** | 마이그레이션 2개 순서대로. ⚠️ **3PL 왕복 먼저** · ⚠️ **R9(심사자 접근 수단)도 여기서 준비** |
...
| 15 | cafe24-fulfillment | **4-1. 서버 — 주문 불러오기** | 대기 | ✗ | | |
| 16 | cafe24-fulfillment | 4-2. 서버 — 전환 (재고 차감 + 역기록) | 대기 | ✗ | | |
| 17 | cafe24-fulfillment | 5-2. 브랜드 화면 — 매핑 | 대기 | ✗ | | |
| 18 | cafe24-fulfillment | 5-3. 브랜드 화면 — 주문 목록 + 전환 | 대기 | ✗ | | |
