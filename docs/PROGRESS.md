...
✅ **카페24 스토어 리스팅(STEP 03 판매정보)의 텍스트는 이미 채워 저장했다**(2026-09-23).
남은 것은 **이미지 4종뿐**이고 전부 배포 뒤에야 만들 수 있다 — 대표아이콘 100×100 ·
대표설명 740×416 · **스크린샷(Mobile 360×640 또는 PC 1920×1080, 최소 1장 필수)** ·
상세설명 본문 이미지 1장. FAQ 는 권장이라 비워 뒀다.

⚠️⚠️ **카페24 자사몰 주문은 "국내 배송"이 정상 경로다**(2026-09-23 사용자 정정). 배송국이
비어서 오는 것이 예외가 아니라 **기본**이고, `Cafe24Connection.defaultCountryCode` 의 기본값
`KR` 이 그래서 맞다. 해외 주소 주문도 보낼 수 있지만 그건 부가 경로다.
→ **코드·브랜드 화면 문구는 이미 이 사실과 맞다**(고칠 것 없음). 스토어 리스팅도 그 기준으로
다시 썼다. ⚠️ 그러나 **계획 문서·결정 로그는 아직 이 트랙을 "해외배송 출고" 축으로 서술**한다 —
정리 여부는 미결이다.

⚠️ **앱 이름과 판매 상품명이 엇갈려 있다** — 개발자센터 앱 이름은 `KLOW 해외배송 출고연동`,
스토어 판매 상품명은 `KLOW 풀필먼트 출고연동`. **앱 이름은 개발자센터 UI 에 수정 경로가 없다**
(`app_name` 이 hidden 필드로만 있고, 그걸 고쳐 저장하면 **Redirect URI·App URL 이 같이 제출돼
OAuth 설정이 깨질 위험**이 있다). 2026-09-23 사용자 결정: **그대로 두고 8단계 제출 전에
카페24에 변경 요청**한다. 브랜드가 보는 **동의 화면에는 옛 이름이 뜬다**는 것을 알고 간다.
