...
**cafe24-fulfillment 4-1단계 — 서버: 주문 불러오기** (2026-09-23 기록)

명세는 트랙 문서 §8 의 4-1 표 행 + §4-C·§4-D·§4-E·§4-G · §5-B. `import`(기간 14일·5페이지
상한 · 미러 upsert · 잘림 처리 · 재수집 정책) · `GET /orders` · `cafe24-retention.cron.ts`
(**cron 11 → 12**).

⚠️⚠️ **착수 전에 반드시 — 테스트 몰에 주문 1건이 있어야 한다.**
`simsgood1` 에 3개월간 주문이 **0건**이라 아래 둘을 **한 번도 못 봤다.**

| 미확정 | 무엇이 막히나 |
|---|---|
| **§2 B** 주문 상태 코드 값 | 어떤 상태를 기본으로 가져올지 정할 수 없다 |
| **§2 F** 옵션 없는 상품의 매핑 키 | 주문 품목의 `variant_code` 가 실값인지 빈 값인지에 따라 **전환(4-2) 조회가 통째로 빗나간다** |

**추측해서 상수로 박지 말 것** — 계획서가 두 곳에서 같은 말을 한다. 주문을 넣을 수 없으면
4-1 을 미루고 **5-2(매핑 화면)** 를 먼저 하는 것도 가능하다(⚠️ 단 §2 F 가 매핑 키를 정하므로
5-2 도 그 결정을 피해갈 수는 없다 — 화면이 짝짓기를 하기 때문이다).

⚠️ **연결은 살아 있다**(테스트 몰 `simsgood1`, refresh 만료 2026-10-07). 터널 없이도 API 호출은
된다 — refresh 는 redirect_uri 를 쓰지 않는다. **새 OAuth 왕복이 필요할 때만** 터널을 다시
띄우고 개발자센터에 Redirect URI 를 추가한다(§2-A).

**배포는 3pl 과 한 번에 낸다**(2026-09-23 결정) — 그 단계는 트랙 문서 §8 의 6단계다.
⚠️ 6단계에서 **R9(심사자 접근 수단)** 도 함께 준비한다 — 없으면 8단계 제출 시점에 막힌다.
...
#### 2-2. 서버 — OAuth 실왕복 + 토큰 갱신 직렬화 · **지금 할 것**

본문은 트랙 문서 [§8 의 2-2](./plan/cafe24-fulfillment/implementation-plan.md) 에 있다. 여기엔 읽을 것과 완료 기준만 둔다.

- ✅ **선행 조건(0단계 자격증명) 충족.** 실측 결과는 트랙 문서 **§2-A**
- ⚠️⚠️ **로컬 왕복에 HTTPS 터널이 필요하다** — Redirect URI 가 HTTP·IP 를 거부한다(§2-A).
  터널 도메인을 개발자센터에 한 줄 추가하고 `CAFE24_BRAND_CALLBACK_URL` 을 같은 값으로 맞춘다
...
#### 0. 카페24 개발자센터 앱 생성 (사용자 작업 · 완료)

앱 `KLOW 해외배송 출고연동`(Web · Korea 스토어). 실측값 전부는 트랙 문서 **§2-A** 에 있다.
✅ **§2 D 판정: 심사가 앱 실행 화면(iframe)을 요구하지 않는다 — §7 R1 회피안 유지.**
...
| 12 | cafe24-fulfillment | 2-2. 서버 — OAuth 실왕복 + 토큰 갱신 | 완료 | ✗ | 2026-09-23 | `1addc5a` / - / - / - / (이 커밋) |
| 15 | cafe24-fulfillment | 3-2. 서버 — 카페24 상품 목록 (`/catalog`) | 완료 | ✗ | 2026-09-23 | `d7b9a55` / - / - / - / (이 커밋) |
| 13 | cafe24-fulfillment | 0. 카페24 개발자센터 앱 생성 (사용자 작업) | 완료 | — | 2026-09-23 | - / - / - / - / (이 커밋) |
