...
**cafe24-fulfillment 2-2단계 — 서버 OAuth 실왕복 + 토큰 갱신 직렬화** (2026-09-23 기록)

2-1(서버 OAuth 왕복 코드)이 끝났다. 명세는 트랙 문서 §8 의 2-2.

- ⚠️⚠️ **여기서부터 0단계 산출물(카페24 자격증명)이 필요하다**(트랙 문서 G2). 아직이면
  **이 단계를 시작하지 말 것** — 실행해 보지 않은 OAuth 코드 위에 또 한 겹을 쌓으면 나중에 어느
  층이 틀렸는지 모른다. 그때는 3단계(매핑 API)의 **순수 로직 부분**을 먼저 하거나 멈춘다.
- **2-1 이 남긴 코드는 한 번도 실행되지 않았다.** 2-2 의 첫 실왕복에서 토큰 교환 URL·응답 모양·
  만료 시각 형식이 처음으로 확인된다.
- cron 10 → 11 이 되므로 `test/app.e2e-spec.ts` 의 개수·이름 목록을 함께 고친다.

**0단계(카페24 개발자센터 앱 생성)는 사용자 작업이고 2-2 의 선행 조건이다.** ⚠️ 앱을 만들 때
**심사 요건에 "앱 실행 화면(iframe)"이 포함되는지 가장 먼저 확인**할 것 — 포함되면 트랙 문서
§7 R1 의 회피안이 무너져 계획을 다시 짜야 한다.
...
| 9 | cafe24-fulfillment | 0. 카페24 개발자센터 앱 생성 (사용자 작업 · **2-2 선행 조건**) | 대기 | — | | |
