...
- ⚠️ **git 은 `feat/3pl-fulfillment` 를 세 레포 `staging` 에 머지만 하고(배포하지 않는다) 그
  `staging` 에서 `feat/cafe24-fulfillment` 를 딴다.**
...
그래서 **DB 브랜치는 `ep-floral-sun` 을 이어 쓰고**(새로 파면 3pl 테이블이 없다), git 은
`feat/3pl-fulfillment` 를 `staging` 에 머지만 한 뒤 거기서 딴다. 트랙 문서 §1-A.
...
- ⚠️⚠️ **그래서 DB 브랜치를 새로 파면 안 된다 — `ep-floral-sun` 을 이어 쓴다.** 3pl 마이그레이션이
  거기에만 있어 staging 기준선에서 새로 파면 `FulfillmentRequest` 가 없다. git 은
  `feat/3pl-fulfillment` 를 `staging` 에 **머지만 하고(배포 X)** 거기서 새 브랜치를 딴다
