...
**cafe24-fulfillment 3-2단계 — 서버: 카페24 상품 목록(`GET /catalog`)** (2026-09-23 기록)

명세는 트랙 문서 §8 의 3-2 표 행. 페이지네이션 · 쿼터 · 429 분기가 범위다.

✅ **0단계·2-2 가 끝났다.** 자격증명이 있고 **OAuth 왕복이 실제로 통과했다** — 실측은 트랙 문서
**§2-A**(개발자센터)와 **§2-B**(OAuth·API)에 있다. 남은 단계는 번호 순서대로 간다.

⚠️⚠️ **먼저 사용자에게 받아야 할 것 두 가지**

1. **HTTPS 터널** — 카페24 API 를 실제로 부르려면 연동이 살아 있어야 한다. Redirect URI 가
   HTTP·IP 를 거부하므로 로컬 왕복에는 터널이 필요하다(§2-A). 터널 주소가 바뀌면 개발자센터
   목록과 `CAFE24_BRAND_CALLBACK_URL` 을 **함께** 고친다. ⚠️ 단, **이미 저장된 연결이 있으면
   터널 없이도 API 호출은 된다** — refresh 는 redirect_uri 를 쓰지 않는다.
2. ⚠️⚠️ **테스트 몰에 주문 1건** — `simsgood1` 에 3개월간 주문이 0건이라 **§2 B(주문 상태 코드)와
   §4-D(품목 단위 필드)를 아직 한 번도 못 봤다.** 4-1·4-2 가 그 값 위에 설계되므로
   **추측해서 상수로 박지 말 것.** 3-2(상품 목록)는 주문과 무관해 먼저 할 수 있다.
