...
**cafe24-fulfillment 2-2단계 — OAuth 실왕복 + 토큰 갱신 직렬화** (2026-09-23 기록)

⚠️⚠️ **막힘이다 — 0단계(카페24 개발자센터 앱 생성)가 먼저다.** 자격증명 없이 앞당길 수 있는
단계 셋(4-0 · 5-1 · 3-1)을 2026-09-23 에 **모두 끝냈다.** 남은 단계는 전부 카페24 API 를
부르므로 여기서 멈춘다.

**사용자가 할 일 — 0단계.** 명세는 아래 `§6` 의 0번. ⚠️ 앱을 만들 때 **심사 요건에
"앱 실행 화면(iframe)"이 포함되는지 가장 먼저 확인**할 것 — 포함되면 트랙 문서 §7 R1 의
회피안이 무너져 계획을 다시 짜야 한다.

자격증명이 도착하기 전에 다른 일을 하려면 **사용자가 우선순위를 정해 준다**(`§1` 절차 1번 —
추측해서 고르지 않는다). 후보는 3pl 트랙 §5 의 백로그와 `§6 일정에 없는 트랙` 이다.

### ⚠️ 자격증명 없이 앞당길 수 있던 세 단계는 **모두 끝났다**

2-1 이 끝난 뒤 **카페24 API 를 한 번도 부르지 않는 단계** 셋을 앞당겼고(2026-09-23 사용자 결정),
같은 날 셋 다 완료했다 — **4-0**(출고신청 목록 기간 필터 · 3PL 부채 R3) · **5-1**(브랜드 연동
화면 · `defaultCountryCode` 선택) · **3-1**(매핑 CRUD).

⚠️ **이제 남은 단계는 전부 0단계 뒤다.** 3-2·4-1·4-2 는 카페24 API 를 부르고, 5-2·5-3 은 그
엔드포인트에 붙는 화면이다. 세 단계가 예외였던 이유는 OAuth 코드 **위에 쌓이지 않아서**
(DB·UI 축)이지 G2 가 느슨해진 게 아니다.

⚠️⚠️ **`/v1/brand/cafe24/*` 는 코드가 있어도 한 번도 실행된 적이 없다.** 연동 화면이 "연결됨"을
그릴 수는 있어도 그 경로가 통과된 적은 없다 — 어디에도 "연동이 된다"고 적지 말 것.
...
#### 2-2. 서버 — OAuth 실왕복 + 토큰 갱신 직렬화 · **지금 할 것 (0단계 대기)**
...
- ⚠️⚠️ **선행 조건: 0단계 산출물(자격증명).** 없으면 시작하지 않는다(트랙 문서 G2).
  앞당길 수 있던 4-0·5-1·3-1 은 **전부 끝났다** — 이 단계 앞에 남은 것은 0단계뿐이다
...
- **완료 기준**
  ...
  - `docs/server/modules/cafe24.md` **갱신**(2-1 이 신규 생성하고 5-1·3-1 이 덧붙였다 —
    재작성이 아니라 갱신이다). ⚠️ 그 문서 머리의 **"실행된 적이 없다"** 경고를 실측으로 바꾸는
    것이 이 단계다
...
| 10 | cafe24-fulfillment | 5-1. 브랜드 화면 — 연동 (자격증명 무관 · 4-2 보다 먼저) | 완료 | ✗ | 2026-09-23 | `73c578a` / - / `0ba23e6` / - / (이 커밋) |
| 11 | cafe24-fulfillment | 3-1. 서버 — 매핑 CRUD (자격증명 무관) | 완료 | ✗ | 2026-09-23 | `1bb91c4` / - / - / - / (이 커밋) |
| 12 | cafe24-fulfillment | 2-2. 서버 — OAuth 실왕복 + 토큰 갱신 | 막힘 (0단계 자격증명 대기, 2026-09-23~ · **앞당길 수 있는 단계가 더 없다**) | ✗ | | |
