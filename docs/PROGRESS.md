...
**그다음: cafe24-fulfillment 0단계** — 카페24 개발자센터 앱 생성. **사용자 작업이고 코드가 없다.**
앱을 만들어 `client_id`/`client_secret` 과 redirect URI 를 확보하고, 트랙 문서 §2 의 **A·B·D**
(설치 진입점의 `mall_id` 파라미터 · 주문 상태 코드 · **심사 요건에 "앱 실행 화면(iframe)"이
포함되는지**)를 실측해 채우면 1단계를 시작할 수 있다. ⚠️ **D 를 가장 먼저 본다** — 포함되면
iframe 회피안(§7 R1)이 무너져 계획을 다시 짜야 한다.
⚠️ **3pl 7단계가 먼저다** — 그 테이블 위에 얹히는 트랙이라 staging 미머지 상태에서 브랜치를 파면
머지 기준선이 둘로 갈린다(트랙 문서 §1 G1).
...
⚠️⚠️ **착수 게이트: 3pl 7단계(운영 배포)가 먼저다.** 트랙 문서 §1 의 G1~G4 를 착수 전에 읽는다
— 특히 **G4(락 획득 순서: 미러 → 재고)** 는 어기면 데드락이라 코드를 쓰기 전에 알아야 한다.
...
#### 0. 카페24 개발자센터 앱 생성 (사용자 작업)

- **읽을 것**: 트랙 문서 `§2`(미확정 A·B), `§6` 의 0단계
- **건드리는 레포 · 배포 순서**: 없음 — **코드 없음**
- **스키마·데이터 위험**: 없음
- **할 일**: 개발자센터 앱 생성(scope `mall.read_order`·`mall.read_product`) · redirect URI 등록
  (⚠️ **로컬 `http://localhost:4000/v1/brand/cafe24/callback` 포함** — 빠지면 개발 중 콜백을 못 받는다)
  · 테스트 쇼핑몰 1개 연결
- **완료 기준**: `client_id`/`client_secret` 확보 + 트랙 문서 `§2` 의 **A·B·D 를 실측값으로 채움**
  (⚠️ **D = 심사 요건에 "앱 실행 화면(iframe)"이 포함되는지** — 포함되면 §7 R1 의 회피안이 무너져
  계획을 다시 짜야 한다. 가장 먼저 확인할 것)
- **해제 조건(사건)**: 앱 생성 + 테스트 몰 1개 설치 + `client_id`/`client_secret` 전달 완료
- **선행 조건**: 3pl 7단계 완료
...
| 8 | cafe24-fulfillment | 0. 카페24 개발자센터 앱 생성 (사용자 작업) | 대기 | — | | |
...
- **다음 단계가 알 것**: 0단계는 **사용자 작업**이고, 그 전에 **3pl 7단계(운영 배포)가 먼저다**
  (G1). cron 은 10 → 12 가 된다(토큰 갱신 · 미러 파기) — `test/app.e2e-spec.ts` 를 함께 고친다
