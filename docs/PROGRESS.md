...
✅ **0단계가 끝나 막힘이 풀렸다.** 자격증명(`client_id`/`client_secret`)을 확보했고, 실측
결과는 트랙 문서 **§2-A** 에 있다. 명세는 아래 `§6` 의 2-2.

⚠️⚠️ **로컬에서 OAuth 를 돌리려면 HTTPS 터널이 필요하다** — 0단계에서 드러난 제약이다.
개발자센터가 Redirect URI 에 **HTTP·IP 를 거부**해 `http://localhost:4000/...` 을 등록할 수
없다(§2-A). 이 레포의 다른 연동(`GOOGLE_*`·`META_*`)은 localhost 를 쓰므로 **그 관례를
복사하면 막힌다.** 터널 주소가 바뀌면 개발자센터 목록과 `CAFE24_BRAND_CALLBACK_URL` 을
**함께** 고친다.

⚠️ **§2 B(주문 상태 코드)가 아직 미확정이다** — 2-2 의 실응답에서 확인해 채운다.

### 남은 순서

0단계가 끝나 **번호 순서대로 간다** — 2-2 → 3-2 → 4-1 → 4-2 → 5-2 → 5-3 → 6(배포) → 7 → 8(심사).
자격증명 없이 앞당겼던 셋(4-0 · 5-1 · 3-1)은 2026-09-23 에 모두 끝났다.

⚠️⚠️ **`/v1/brand/cafe24/*` 는 코드가 있어도 한 번도 실행된 적이 없다.** 연동 화면이 "연결됨"을
그릴 수는 있어도 그 경로가 통과된 적은 없다 — **2-2 가 그 사실을 바꾸는 단계**이고, 그전까지
어디에도 "연동이 된다"고 적지 말 것.
...
| 12 | cafe24-fulfillment | 2-2. 서버 — OAuth 실왕복 + 토큰 갱신 | 진행 중 | ✗ | | |
