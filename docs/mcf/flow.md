...
## 7. 가격·정산 — MCF 채널 트랙 (채널별 고정 판매가·역산 정산가)

가격 정본은 **고정 판매가**이고 정산가/마진은 그 판매가에서 **역산**된다(→ `pricing-model.md`). MCF 는 "보전(reimbursement)" 이 아니라 **채널별 판매가·정산 트랙**으로 처리한다. Amazon 은 EFS 국제 물류비가 안 드는 대신 **Amazon 이 브랜드 셀러 계정에서 자기 MCF 수수료를 뗀다** — 그래서 브랜드가 그 몫을 **MCF 판매가/정산가**에 미리 반영해 값을 잡는다. (핵심: KLOW 가 Amazon 수수료를 조회·보전하지 않는다.)

### 7-1. 판매가·정산가는 채널별 2벌 (고정 판매가 → 정산가 역산)
| | 판매가(고객, 고정) | 정산가(브랜드, 역산) |
|---|---|---|
| 일반(EFS) | 고정 판매가 | 판매가에서 **물류비/2 차감** 역산 (`floor(customerUsd×fx×0.95 − 물류비/2)`) |
| MCF | 고정 판매가(기본 = 일반과 동일) | 판매가에서 역산하되 **물류비/2 차감 없음** (`floor(customerUsd×fx×0.95)`) |

- **MCF 판매가 기본값 = 일반 고정 판매가와 동일**. 다만 MCF 정산가는 EFS 물류비가 안 드니 **물류비/2 차감을 생략**해 역산하므로, 기본값이 **일반 정산가보다 물류비/2 만큼 높다**(브랜드가 그 몫을 마진으로 자연 흡수). 넷 효과는 옛 "MCF마진 = 마진 + 물류비/2" 와 동일하다.
- 브랜드는 그 안에서 자기 Amazon 수수료를 감안해 값을 정한다. `ProductAmazonListing.mcfMarginKrw`(§1)는 MCF 트랙을 **독립 조절**하는 선택 오버라이드 — null 이면 위 기본값(판매가=일반, 정산가=물류비/2 미차감 역산).
- 정확한 식은 기존 `product-selects.ts priceLine()`·÷0.95 PG·배송비 라인과 대조해 확정한다 — **특히 MCF 주문에서 별도 배송비 라인을 0 으로 둘지**(정산가 물류비/2 미차감과 짝을 이룸).

### 7-2. Amazon 수수료는 브랜드가 부담 (KLOW 보전·조회 없음)
- Amazon MCF 이행 수수료는 **재고 소유자=브랜드의 셀러 계정에서 자동 차감**된다. **KLOW 엔 청구 안 됨**(KLOW 는 SP-API 사용료만 별도).
- 브랜드가 이 수수료를 **MCF 판매가/정산가에 미리 반영**해 값을 책정하므로, KLOW 가 실제 수수료를 **조회하거나 보전할 필요가 없다**
...
- 브랜드 정산액 = `Σ (그 라인이 실제 나간 채널의 정산가) × qty`. MCF 로 나갔으면 MCF 정산가(물류비/2 미차감 역산), EFS(폴백 포함)면 일반 정산가(물류비/2 차감 역산).
...
- (기본값에선 MCF·일반 판매가가 같아 고객가 체감은 동일. `mcfMarginKrw` 로 MCF 트랙을 조절하면 MCF 판매가만 독립적으로 달라진다.)
