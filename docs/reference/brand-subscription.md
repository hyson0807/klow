...
|---|---|---|---|
| 6개월납 (`semiannual`) | ₩360,000 | **₩396,000** | ₩66,000 |
| 연납 (`annual`)  | ₩600,000 | **₩660,000** | ₩55,000 |
...
- 단일 플랜이라 `SubscriptionPlan` 테이블을 만들지 않고 **코드 상수 + 환경변수**로 관리한다:
  ...
  - `SUBSCRIPTION_SEMIANNUAL_PRICE_KRW=396000` (미설정·비정상 시 코드 기본값 396000 + WARN 로그)
  - `SUBSCRIPTION_ANNUAL_PRICE_KRW=660000`
...
SUBSCRIPTION_SEMIANNUAL_PRICE_KRW=396000
SUBSCRIPTION_ANNUAL_PRICE_KRW=660000
SUBSCRIPTION_PLAN_CODE=brand_standard
SUBSCRIPTION_ORDER_NAME_PREFIX=KLOW 브랜드 멤버십
```
...
- **가입 진입 흐름** (`studio/page.tsx` 오케스트레이션):
  ...
  2. `PaymentWallPanel` 의 `needsPaymentWall(brand)` = `needsOnboarding && !!pgCustomerKey` → 같은 탭이 자동으로
     paywall 로 전환. KLOW PRO 플랜(6개월 월환산 ₩60,000 / 연 월환산 ₩50,000, `billingInterval` 토글) + 자동결제 동의
