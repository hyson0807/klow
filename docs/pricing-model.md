...
- **klow_brand · klow_admin**: 제품 편집에서 **원가(KRW) + 기본 판매가(USD) + 국가별 현지통화 핀**을 입력. `src/lib/cost-pricing.ts`가
  고정가 → **정산가(KRW) 역산 + 마진 + 원가 밑 경고**를 입력 즉시 미리보기(서버 공식 미러). 핀은 현지통화로 입력·표시.
