...
- **요율의 캐리어는 무게별 최저가**: EFS·EMS 둘 다 가능하면 그 무게에서 더 싼 쪽(동점 → EFS, EFS
  제외구역이면 EMS), 둘 다 불가면 DHL. EFS 가 가능해도 EMS 가 더 싸면 EMS 요율을 쓴다.
  무게 티어/요율·캐리어 선택 구조는 [`klow_server/docs/modules/shipping.md`](../klow_server/docs/modules/shipping.md) 참고.
...
- 무게 요율·캐리어: `pickTierRate` / `pickCheapestCarrier`(무게별 EFS·EMS 최저가) /
  `loadCheapestRatePlanner`(주문·시딩 정본) / `resolveRatesForWeight`(미리보기 국가별 맵) —
