...
- (미러 컬럼 `ShippingCountry.{efs,ems,dhl}RateKrw`·무게 티어 `ShippingRate`·kg당 추가요금
  `efs/emsSurcharge…`·엑셀 업로드·어드민 무게 매트릭스는 **크리에이터 시딩 박스 비용 산정 전용**으로 유지된다 —
  제품 가격 경로에서는 더 이상 쓰이지 않는다.) 요율·캐리어 구조는
