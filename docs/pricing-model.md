...
| **기본 판매가** (전국가) | 브랜드/어드민 입력 | USD 센트 | `Product.basePriceUsd` **저장** | 신 가격 정본. 핀 없는 국가에 적용(고정) |
...
| `salePrice` (legacy) | — | KRW | `Product.salePrice` 저장(호환값) | 구 모델 잔재. 신 모델 미사용 |
| **손님 판매가** (취소선) | 서버 계산 | USD | 응답 `listPriceUsd`(센트) | 핀 국가 `priceLocal/rate`, 미핀 `basePriceUsd`. 목적국별 |
| **실제 결제가** | 서버 계산 | USD | 응답 `customerPriceUsd`(센트) | 할인 적용가 |
| 정산가 (개당 브랜드 몫) | 서버 역산 | KRW | 응답엔 미노출(편집기만) | `= 청구USD × fx × 0.95 − 물류비/2`. **유동**(표시 시점 환율) |

> 핀도 할인도 없는 국가는 `ProductCountryPrice` 행을 만들지 않고 기본(basePriceUsd, 할인=글로벌 discount)을 상속한다.
> 완성도 게이트(`PUBLIC_PRODUCT_WHERE`)는 `image != '' && basePriceUsd > 0` — 기본 판매가를 채워야 노출/판매된다.
...
- `attachCustomerPricing(row, fxRate, ctx)` / `resolvePricingCtx(prisma, country)`(→ logisticsKrw + **currencyUsdRate**) /
  `writeProductCountryPrices(tx, productId, rows)` — `product-selects.ts`. 공개 응답은 `costKrw`/`countryPrices`/`basePriceUsd` strip.
...
- **입력 방식은 두 앱이 다르지만 저장 정본은 동일**(basePriceUsd + 국가별 priceLocal 핀). `src/lib/cost-pricing.ts`가
  ...
  - **klow_brand**: **원가(KRW) + 마진(KRW) 입력** → 기준 판매가(USD)를 **US 물류비 기준으로 파생**(basePriceUsd). 국가별은
    판매가 모달에서 **그 나라 현지통화 판매가를 직접 입력**해 `priceLocal` 핀으로 그대로 저장(마진 왕복 없음 → 딱 떨어진 값 유지,
    실제 청구 USD만 근사 센트). 카드의 국가별 마진은 그 판매가에서 역산한 **읽기전용** 표시. 환율 소스는 `/v1/shop/fx-rate`.

## 스키마

```prisma
model Product {
  basePriceUsd  Int?     // 전국가 기본 판매가(USD 센트, 고정) — 신 가격 정본
  costKrw       Int?     // 원가(KRW) — 원가 밑 경고 기준
  salePrice     Int      // [legacy] 구 정산가. 호환용
...
model ProductCountryPrice {
  ...
  marginKrw   Int?     // [dormant] 구 모델 마진. priceLocal 백필 소스 — 드롭 예정
