...
> **`500g 요율 그대로`**(절반 나눗셈 폐지)로 바뀌었다 — `배송비 = 요율표500g / fx × 청구 대상 브랜드수`.
> ⚠️ 요율표는 고정비 비중이 커서 **무게 비례가 아니다**(500g 요율 ≠ 1kg 요율의 절반) — 이 전환으로
> 배송비가 **98개국 중 96개국에서 올랐다**(중앙값 +24%; 내려간 곳은 US·PH 둘뿐).
> 당시엔 브랜드 후청구액(`max(0, 실비+수수료−선결제)`)이 그만큼 줄었다 — ⚠️ **2026-09 전환으로
> 선결제 차감이 폐지**되어 지금은 후청구액이 배송비 요율과 무관하고, 대신 그 인상분이 브랜드
> 정산금으로 간다(아래 2026-09 전환).
> 공개 응답 필드는 `logisticsCost2kgKrw` → **`customerShippingKrw`**(무게 중립 이름)로 rename됐고,
> 서버가 과도기 동안 구 이름을 같은 값으로 병기한다. 캐리어(EFS/EMS) 근사 판정만 **2kg 를 유지**한다
> (`CARRIER_FALLBACK_WEIGHT_G` — 낮추면 무거운 박스를 EFS 로 오판). `Order.shippingFeeByBrand`
> 스냅샷 덕에 **기존 주문은 소급 영향 없음**이고 마이그레이션·백필도 없다.
>
...
> 고객이 결제하는 **배송비는 그대로 남았고**(당시 기준 `물류비/2 × 청구 대상 브랜드수` — 기준 무게는
> 위 2026-07-30 전환으로 바뀌었다), 당시 그 선결제분은 브랜드 실측 물류비의 **선납금**이었다
> (입고 실측과의 차액만 후청구). ⚠️ **2026-09 전환으로 이 관계가 끊겼다** — 아래 참조.
...
- **브랜드 정산가(KRW) 역산** = `floor(customerPriceUsd/100 × usdKrwRate × (1 − 0.05))`.
  PG 5% 만 차감한 순정산액 — **물류비는 차감하지 않는다**(배송비는 고객이 별도 결제하고, 그 결제액도
  같은 산식으로 브랜드에 정산된다 — 아래 2026-09 전환).
