...
| 개념 | 누가 정함 | 통화 | 저장 위치 | 비고 |
|---|---|---|---|---|
| **브랜드 정산가** (브랜드가 받는 돈) | 브랜드 입력 | KRW | `Product.salePrice` | 브랜드가 입력하는 유일한 가격 |
| 고객 할인율 | 어드민 입력 (브랜드는 0) | % | `Product.discount` | 고객가에 직접 적용되는 마케팅 할인 |
| **고객 노출 판매가** (실제 결제가) | 서버 계산 | USD | **저장 안 함** → 응답 `customerPriceUsd`(센트) | `(salePrice/fx + $15)/(1−0.05)` |
| 취소선 원가 (마케팅 앵커) | 서버 계산 | USD | 저장 안 함 → 응답 `listPriceUsd`(센트) | `customerPriceUsd ÷ (1 − discount/100)` |
| 고객 화면 할인율 | 서버 전달 | % | 응답 `customerDiscountPercent` | `= discount` (고객가 기준이라 그대로) |
| 고객 결제 단가 스냅샷 | 주문 시 고정 | USD | `OrderItem.unitPriceUsd`(센트) | 주문 순간 customerPriceUsd 박음 |
| 정산가 스냅샷 (브랜드 지급 기준) | 주문 시 고정 | KRW | `OrderItem.settlementPriceKrw` | `= 주문 시점 salePrice` |
| **고객 결제 총액 = PG 청구액** | 주문 시 고정 | USD | `Order.totalUsd`(센트) | `Σ unitPriceUsd×qty + 배송비` |
| 배송비 | 캐리어 설정 | USD | `Order.shippingFeeUsd`(센트) | `feeUsd × distinct brandId 수` |
| 주문시점 환율 | 주문 시 고정 | — | `Order.fxRateSnapshot` | 결제·환불·정산 환산 기준 |
| 운영 환율 | 어드민 설정 | — | `ShopSettings.usdKrwRate` | `/v1/shop/fx-rate`, 위 계산 입력 |
