...
| B | **주문 상태 코드 값**과 기본으로 가져올 상태 | △ **모양은 확인, 코드 목록은 미완**(2026-09-23 실측, §2-C). 상태는 **품목 단위**다(`order_status`·`status_code`·`status_text`). ⚠️⚠️ **주문 최상위에는 `order_status` 가 없다** — 최상위는 `paid`/`canceled`/`shipping_status` 뿐이다. 관측된 값은 `입금전`(`order_status:"N00"` / `status_code:"N1"`) 하나뿐이라 **기본 필터를 정하지 않는다** — 4-1 은 **기간으로만** 불러오고 상태는 미러에 저장해 화면이 고르게 한다 |
...
#### ⚠️⚠️ 상태는 품목 단위다 — 주문 최상위에 `order_status` 가 없다

| 위치 | 칸 | 관측값 |
|---|---|---|
| **품목** | `order_status` | `"N00"` |
| **품목** | `status_code` / `status_text` | `"N1"` / `"입금전"` |
| 주문 최상위 | `paid` / `canceled` / `shipping_status` | `"F"` / `"F"` / `"F"` |

즉 §3 의 *"주문 상태는 품목 단위다"* 가 실측으로 확인됐다. ⚠️ **최상위에서 상태를 읽으려 하면
칸 자체가 없다.** `order_item_code` 는 `20260923-0000014-01` 꼴(주문번호 + 순번)이다.

⚠️ **관측된 상태가 `입금전` 하나뿐이라 "기본으로 불러올 상태"를 정하지 않는다**(§2 B) —
4-1 은 **기간으로만** 불러오고, 상태는 `status_text` 까지 미러에 저장해 **화면이 고르게** 한다.
코드 목록을 추측해 상수로 박으면 그 추측이 필터에 굳는다.
