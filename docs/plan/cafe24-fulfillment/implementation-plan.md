...
| # | 게이트 | 어기면 |
|---|---|---|
| G1 | **3PL 트랙 7단계(운영 배포)가 `완료` + 운영 배포 `✓` 여야 한다** | 마이그레이션 `20260922070013_add_3pl_fulfillment` 가 전용 Neon 브랜치 `ep-floral-sun` 에만 있고 세 레포가 `feat/3pl-fulfillment` 에 있다. 그 위에서 또 브랜치를 파면 **두 트랙의 마이그레이션이 같은 DB 브랜치에 쌓이고**([PROGRESS §4](../../PROGRESS.md) WIP 상한 — *마이그레이션 동반 단계는 동시에 1개*) 머지 순서를 사람이 기억해야 한다 |
| G2 | **0단계(앱 생성)가 끝나야 2단계를 시작한다** | `client_id`/`client_secret`·등록된 redirect URI 없이는 OAuth 를 한 번도 실행해 볼 수 없다 |
| G3 | **스키마를 바꾸는 단계는 git 브랜치 + Neon DB 브랜치를 함께 판다** | 공유 dev DB 에 미병합 마이그레이션이 남으면 다른 브랜치가 리셋을 요구받는다([`CLAUDE.md`](../../../CLAUDE.md) Prisma 절) |
...
| # | 항목 | 잠정 처리 |
|---|---|---|
| A | **앱스토어 설치 진입점에서 `mall_id` 가 어떤 파라미터로 오는지** | 0단계에서 실측. ⚠️ 그보다 큰 문제가 §7 R1(iframe 쿠키)이라, **v1 은 앱스토어 진입을 지원하지 않는다** |
| B | **주문 상태 코드 값**과 기본으로 가져올 상태 | 0단계 이후 실측. 그때까지 상태를 **브랜드가 고르게** 두고 기본값을 비운다. ⚠️ 코드 값을 소스 상수로 박기 전에 문서에서 확인 |
| C | **주문 목록 조회의 날짜 범위 최대 일수** | 요청당 100건은 실측됐다. 기간 상한은 미확인 → 우리 쪽에서 **14일 상한**을 먼저 건다(§5) |
| D | **퍼블릭앱 심사 제출물·기간** · 심사 요건에 "앱 실행 화면(iframe)"이 포함되는지 | 0단계에서 확인. ⚠️ **포함되면 §7 R1 의 회피안이 무너진다** — 그때 계획을 다시 짠다 |
...
세 테이블 모두 소규모이고 `FulfillmentRequest` 는 운영 데이터가 0건이라(G1 이후 배포 직후) 실질
무해하다. **롤링 안전 · 백필 없음**은 유지된다.
...
⚠️ **심사 요건에 "앱 실행 화면"이 포함되면 이 회피안이 무너진다** — 0단계에서 확인한다(§2 D).
...
### 0. 카페24 개발자센터 앱 생성 (사용자 작업 — 코드 없음 · **막힘 행**)

- **할 일**: 개발자센터 계정 → 앱 생성 → scope `mall.read_order`·`mall.read_product`(+`mall.read_store`)
  → redirect URI 등록 → 테스트 쇼핑몰 1개 연결
- **redirect URI**: 로컬 `http://localhost:4000/v1/brand/cafe24/callback` + 스테이징/운영 도메인.
  ⚠️ **로컬을 빼면 개발 중 한 번도 콜백을 못 받는다**
- **완료 기준**: `client_id`·`client_secret` 확보 + **§2 의 A·B·D 를 실측값으로 채움**
- **해제 조건(사건)**: 앱 생성 + 테스트 몰 1개 설치 + client_id/secret 전달 완료
- **스키마·데이터 위험**: 없음
...
- **스키마·데이터 위험**: `CREATE TABLE` ×4 + `CREATE TYPE` ×1 + `ADD COLUMN` ×1(`source`,
  ...
  DROP 0건 · 백필 없음 → **롤링 안전**. ⚠️ **git 브랜치 + 전용 Neon DB 브랜치를 함께 판다**(G3)
...
- **완료 기준**
  ...
  - `npm run test:e2e` 통과(cron 은 아직 10개 — 2-2 에서 는다)
- **불변식**: G2

### 2-2. 서버 — 토큰 갱신 직렬화 + 첫 실호출
...
| # | 단계 | 범위 |
|---|---|---|
| 3 | 서버 — 상품 매핑 API | `GET /catalog`(페이지네이션·쿼터) · 매핑 upsert/삭제(**보낸 줄만**) |
...
| 5-1 | 브랜드 화면 — 연동 | 출고신청 서브탭 액션 툴바 2×2(**+ `EmptyState` 중복 정의**) · 연결/해제 · `needsReauth` 배너 |
...
| 6 | 운영 배포 | `klow_server → klow_admin → klow_brand`. 뒤집으면 `/v1/brand/cafe24/*` 404 |
| 7 | 실브랜드 1~2곳 시범 | **5개 몰 한도 안에서.** 심사 전에도 실사용이 된다 |
| 8 | 퍼블릭앱 심사 제출 | **외부 대기 = 막힘 행.** 심사자는 동작하는 앱을 본다 — 6·7 이 먼저다 |
