...
| B | **주문 상태 코드 값**과 기본으로 가져올 상태 | ⚠️⚠️ **여전히 미확정 — 실측에 실패했다.** 2-2 에서 주문 API 호출은 성공했으나(HTTP 200) **테스트 몰 `simsgood1` 에 최근 3개월 주문이 0건**이라 상태 코드를 한 번도 못 봤다. **4-1 착수 전에 테스트 주문을 한 건 넣고 실측한다.** 그때까지 상태를 **브랜드가 고르게** 두고 기본값을 비운다 |
...
| F | **옵션 없는 상품의 매핑 키** — `variant_code` 실값인가 빈 문자열인가 | ⚠️⚠️ **미확정.** 실측상 **옵션이 없는 상품도 `variants` 가 1줄 오고 `variant_code` 가 실재한다**(`P00000BY000A`, `options: null`). 그런데 `Cafe24ProductMap.variantCode` 주석은 "옵션 없으면 빈 문자열"이라고 적혀 있다. **주문 품목이 어느 값을 싣는지 못 봤으므로**(§2 B — 주문 0건) 지금 정하면 전환(4-2) 조회가 통째로 빗나갈 수 있다. → 3-2 의 `/catalog` 는 **짝짓기를 하지 않고** 상품 단위로 매핑을 그대로 실어 보낸다. **5-2 착수 전에 주문 품목의 `variant_code` 를 실측해 정한다** |
