...
KLOW 입점 브랜드가 자기 도메인(예: `shop.brandA.com`)을 연결하면, 그 주소에서 **브랜드관 → 제품
상세 → 장바구니**가 동작하고 **로그인·결제·주문조회는 `klow.kr`** 로 이어지게 하는(핸드오프)
기능의 설계 문서 모음.

> 상태: [진행표](../../PROGRESS.md) 참조 (이 트랙은 `§6 일정에 없는 트랙` 에 있다).
> ⚠️ **어디까지 했는가를 이 폴더의 어느 문서에도 적지 않는다** — 한때 이 README 와
> `purchase-plan.md` 와 진행표가 P6 상태를 서로 다르게 말했다.
> 배포 절차는 [implementation-plan §7 배포 순서](./implementation-plan.md#7-배포-순서) ·
> [배포 런북](./deploy-runbook.md).
>
> **P6 대행 구매** — KLOW 가 도메인을 대신 사서 자동 연결하고 연 이용료를 받는다
> (브랜드는 DNS 를 만지지 않는다). 정본은 [`purchase-plan.md`](./purchase-plan.md).
> ⚠️ 그 문서 상단의 **점검 블록 4개를 먼저 읽을 것** — 되돌리면 돈을 잃는 결정들이 거기 모여 있다.
> 특히 **구매는 `FOR UPDATE` 로 직렬화**하고 · **`netCancel` 불확정 건을 `failed` 로 접지 않으며** ·
> **`autoRenew=true` 인 행은 `expiresAt` 이 지나도 정리하지 않는다**(갱신 확인이 안 끝난 상태다) ·
> **환불 금지 판정은 계정이 아니라 registration 행 단위다**.
>
> ⚠️ 스테이징에서 **기능이 통째로 죽은 사건이 한 번 있었다** — 미들웨어 resolve 타임아웃이
> 1초였는데 실제 왕복이 1.2~1.6초라 모든 요청이 abort 됐다(F30). 지금은 3초다.
>
> 설계·통합 지점은 실측 코드와 대조해 확정했다.
