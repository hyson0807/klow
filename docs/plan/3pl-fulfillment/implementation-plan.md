...
| 7 | 운영 배포 (마이그레이션 적용 → server → admin → brand) | 전 레포 |
...
> 3~6단계는 구현이 끝났다. **지은 것의 정본은 [`../../server/modules/fulfillment.md`](../../server/modules/fulfillment.md)**
> 이고 아래는 착수 당시의 스펙이다 — 현행 동작으로 인용하지 말 것.
