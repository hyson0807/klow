...
>
> **코드 위치**: klow_brand 의 `Amazon 창고` 페이지(목업)는 브랜치 **`develop/mcf2`** 에만 있다
> — ⚠️ staging 미머지라 현 체크아웃엔 `amazon/` 페이지도 `amazon-mock.ts` 도 없다.
> 설계·통합 지점은 실측 코드와 대조해 확정했으므로 이 문서만 보고 구현에 들어갈 수 있다.
>
